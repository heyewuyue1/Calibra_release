...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=3705]
...
Input [1]: [id#30]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=3709]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[1, int, true] as bigint)),false), [plan_id=3713]

(16) Scan parquet spark_catalog.imdb_10x.title
Output [3]: [id#39, title#40, kind_id#42]
Batched: true
Location: InMemoryFileIndex [file:/path/to/datasets/imdb_10x_parquet/title]
PushedFilters: [IsNotNull(title), Not(EqualTo(title,)), Or(StringContains(title,Champion),StringContains(title,Loser)), IsNotNull(id), IsNotNull(kind_id)]
ReadSchema: struct<id:int,title:string,kind_id:int>

(17) Filter
Input [3]: [id#39, title#40, kind_id#42]
Condition : ((((isnotnull(title#40) AND NOT (title#40 = )) AND (Contains(title#40, Champion) OR Contains(title#40, Loser))) AND isnotnull(id#39)) AND isnotnull(kind_id#42))
...
Input [1]: [id#293]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=3716]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[1, int, true] as bigint), 32) | (cast(input[2, int, true] as bigint) & 4294967295))),false), [plan_id=3720]
...
Input [1]: [id#210]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=3723]
...
Arguments: hashpartitioning(movie_id#33, movie_id#218, id#39, 200), ENSURE_REQUIREMENTS, [plan_id=3731]
...
(40) Filter
Input [2]: [id#320, info#321]
Condition : ((isnotnull(info#321) AND (info#321 = release dates)) AND isnotnull(id#320))
...
Input [1]: [id#320]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=3726]
...
Input [3]: [movie_id#213, info_type_id#214, id#320]

(45) Exchange
Input [1]: [movie_id#213]
Arguments: hashpartitioning(movie_id#213, movie_id#213, movie_id#213, 200), ENSURE_REQUIREMENTS, [plan_id=3732]
...
Aggregate Attributes [3]: [min#326, min#327, min#328]
Results [3]: [min#329, min#330, min#331]

(50) Exchange
Input [3]: [min#329, min#330, min#331]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=3739]

(51) SortAggregate
Input [3]: [min#329, min#330, min#331]
Keys: []
Functions [3]: [min(name#24), min(info#220), min(title#40)]
Aggregate Attributes [3]: [min(name#24)#322, min(info#220)#323, min(title#40)#324]
Results [3]: [min(name#24)#322 AS producing_company#312, min(info#220)#323 AS rating#313, min(title#40)#324 AS movie_about_winning#314]

(52) AdaptiveSparkPlan
Output [3]: [producing_company#312, rating#313, movie_about_winning#314]
Arguments: isFinalPlan=false
Execution Time: 23.944
