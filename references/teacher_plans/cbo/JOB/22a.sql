...
(7) BroadcastExchange
Input [1]: [id#210]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=15381]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=15385]
...
Condition : (((isnotnull(production_year#43) AND (cast(production_year#43 as int) > 2008)) AND isnotnull(id#39)) AND isnotnull(kind_id#42))
...
Input [1]: [id#293]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=15388]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, true] as bigint) & 4294967295))),false), [plan_id=15392]
...
Input [1]: [id#30]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, false] as bigint)),false), [plan_id=15395]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[4, int, true] as bigint)),false), [plan_id=15399]
...
Arguments: HashedRelationBroadcastMode(List(input[0, int, true], input[3, int, true], input[1, int, true]),false), [plan_id=15403]
...
(42) Filter
Input [2]: [id#1051, info#1052]
Condition : ((isnotnull(info#1052) AND (info#1052 = rating)) AND isnotnull(id#1051))
...
Input [1]: [id#1051]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=15406]

(45) BroadcastHashJoin
Left keys [1]: [info_type_id#219]
Right keys [1]: [id#1051]
Join type: Inner
Join condition: None

(46) Project
Output [7]: [movie_id#213, id#39, title#40, movie_id#33, name#24, movie_id#218, info#220]
Input [9]: [movie_id#213, id#39, title#40, movie_id#33, name#24, movie_id#218, info_type_id#219, info#220, id#1051]
...
Input [1]: [id#110]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=15409]
...
Arguments: HashedRelationBroadcastMode(List(input[0, int, true], input[0, int, true], input[0, int, true], input[0, int, true]),false), [plan_id=15413]
...
Aggregate Attributes [3]: [min#1057, min#1058, min#1059]
Results [3]: [min#1060, min#1061, min#1062]

(59) Exchange
Input [3]: [min#1060, min#1061, min#1062]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=15418]

(60) SortAggregate
Input [3]: [min#1060, min#1061, min#1062]
Keys: []
Functions [3]: [min(name#24), min(info#220), min(title#40)]
Aggregate Attributes [3]: [min(name#24)#1054, min(info#220)#1055, min(title#40)#1056]
Results [3]: [min(name#24)#1054 AS movie_company#1043, min(info#220)#1055 AS rating#1044, min(title#40)#1056 AS western_violent_movie#1045]

(61) AdaptiveSparkPlan
Output [3]: [movie_company#1043, rating#1044, western_violent_movie#1045]
Arguments: isFinalPlan=false
Execution Time: 31.483
