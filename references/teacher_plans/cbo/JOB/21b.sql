               ...
               +- BroadcastExchange (43)
                  +- Project (42)
                     +- BroadcastHashJoin Inner BuildRight (41)
                        ...
                        :  :     +- Project (32)
                        :  :        +- Filter (31)
                        :  :           +- Scan parquet spark_catalog.imdb_10x.title (30)
                        :  +- Filter (36)
                        :     +- Scan parquet spark_catalog.imdb_10x.movie_link (35)
                        +- BroadcastExchange (40)
                           +- Filter (39)
                              +- Scan parquet spark_catalog.imdb_10x.link_type (38)
...
(6) BroadcastExchange
Input [1]: [id#110]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=14644]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=14648]
...
Input [1]: [id#30]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=14651]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[2, int, true] as bigint)),false), [plan_id=14655]
...
PushedFilters: [In(info, [German,Germany]), IsNotNull(movie_id)]
ReadSchema: struct<movie_id:int,info:string>

(27) Filter
Input [2]: [movie_id#213, info#215]
Condition : (info#215 IN (Germany,German) AND isnotnull(movie_id#213))
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=14658]

(30) Scan parquet spark_catalog.imdb_10x.title
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, true] as bigint) & 4294967295))),false), [plan_id=14661]

(35) Scan parquet spark_catalog.imdb_10x.movie_link
...
(38) Scan parquet spark_catalog.imdb_10x.link_type
...
Input [2]: [id#113, link#114]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, false] as bigint)),false), [plan_id=14664]
...
Input [5]: [movie_id#213, id#39, title#40, movie_id#119, link#114]
Arguments: HashedRelationBroadcastMode(List(input[0, int, true], input[0, int, true], input[3, int, true], input[3, int, true], input[1, int, true], input[1, int, true]),false), [plan_id=14668]
...
Aggregate Attributes [3]: [min#1012, min#1013, min#1014]
Results [3]: [min#1015, min#1016, min#1017]

(47) Exchange
Input [3]: [min#1015, min#1016, min#1017]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=14673]

(48) SortAggregate
Input [3]: [min#1015, min#1016, min#1017]
Keys: []
Functions [3]: [min(name#24), min(link#114), min(title#40)]
Aggregate Attributes [3]: [min(name#24)#1009, min(link#114)#1010, min(title#40)#1011]
Results [3]: [min(name#24)#1009 AS company_name#1001, min(link#114)#1010 AS link_type#1002, min(title#40)#1011 AS german_follow_up#1003]

(49) AdaptiveSparkPlan
Output [3]: [company_name#1001, link_type#1002, german_follow_up#1003]
Arguments: isFinalPlan=false
Execution Time: 18.587
