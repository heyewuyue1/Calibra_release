...
(6) BroadcastExchange
Input [1]: [id#23]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=17873]
...
Input [1]: [id#30]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, false] as bigint)),false), [plan_id=17877]
...
Input [1]: [movie_id#33]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=17881]
...
Condition : (((isnotnull(production_year#43) AND (cast(production_year#43 as int) > 1990)) AND isnotnull(id#39)) AND isnotnull(kind_id#42))
...
PushedFilters: [In(kind, [movie,tv movie,video game,video movie]), IsNotNull(id)]
ReadSchema: struct<id:int,kind:string>

(20) Filter
Input [2]: [id#293, kind#294]
Condition : (kind#294 IN (movie,tv movie,video movie,video game) AND isnotnull(id#293))

(21) BroadcastExchange
Input [2]: [id#293, kind#294]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, false] as bigint)),false), [plan_id=17884]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, true] as bigint) & 4294967295))),false), [plan_id=17888]
...
Arguments: hashpartitioning(keyword_id#117, 200), ENSURE_REQUIREMENTS, [plan_id=17892]
...
(32) Exchange
Input [1]: [id#110]
Arguments: hashpartitioning(id#110, 200), ENSURE_REQUIREMENTS, [plan_id=17893]
...
Arguments: hashpartitioning(movie_id#33, movie_id#116, id#39, 200), ENSURE_REQUIREMENTS, [plan_id=17903]
...
Input [1]: [id#210]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=17898]
...
Arguments: hashpartitioning(movie_id#213, movie_id#213, movie_id#213, 200), ENSURE_REQUIREMENTS, [plan_id=17904]
...
Arguments: hashpartitioning(movie_id#33, movie_id#33, movie_id#33, movie_id#33, 200), ENSURE_REQUIREMENTS, [plan_id=17914]
...
Input [1]: [id#930]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=17908]
...
Arguments: hashpartitioning(cast(movie_id#927 as int), cast(movie_id#927 as int), cast(movie_id#927 as int), cast(movie_id#927 as int), 200), ENSURE_REQUIREMENTS, [plan_id=17913]
...
Aggregate Attributes [2]: [min#1182, min#1183]
Results [2]: [min#1184, min#1185]

(65) Exchange
Input [2]: [min#1184, min#1185]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=17921]

(66) SortAggregate
Input [2]: [min#1184, min#1185]
Keys: []
Functions [2]: [min(kind#294), min(title#40)]
Aggregate Attributes [2]: [min(kind#294)#1180, min(title#40)#1181]
Results [2]: [min(kind#294)#1180 AS movie_kind#1173, min(title#40)#1181 AS complete_us_internet_movie#1174]

(67) AdaptiveSparkPlan
Output [2]: [movie_kind#1173, complete_us_internet_movie#1174]
Arguments: isFinalPlan=false
Execution Time: 37.698
