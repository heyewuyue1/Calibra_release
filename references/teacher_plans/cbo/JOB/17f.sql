...
(6) BroadcastExchange
Input [1]: [id#110]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=9624]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=9628]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, false] as bigint) & 4294967295))),false), [plan_id=9631]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[3, int, false] as bigint)),false), [plan_id=9634]
...
Arguments: HashedRelationBroadcastMode(List(input[2, int, true], input[0, int, true], input[1, int, true]),false), [plan_id=9638]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=9642]
...
PushedFilters: [IsNotNull(name), StringContains(name,B), IsNotNull(id)]
ReadSchema: struct<id:int,name:string>

(29) Filter
Input [2]: [id#540, name#541]
Condition : ((isnotnull(name#541) AND Contains(name#541, B)) AND isnotnull(id#540))
...
Aggregate Attributes [1]: [min#690]
Results [1]: [min#691]

(33) Exchange
Input [1]: [min#691]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=9647]

(34) SortAggregate
Input [1]: [min#691]
Keys: []
Functions [1]: [min(name#541)]
Aggregate Attributes [1]: [min(name#541)#689]
Results [1]: [min(name#541)#689 AS member_in_charnamed_movie#683]

(35) AdaptiveSparkPlan
Output [1]: [member_in_charnamed_movie#683]
Arguments: isFinalPlan=false
Execution Time: 67.082
