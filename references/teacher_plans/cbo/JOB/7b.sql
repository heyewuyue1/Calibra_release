...
(7) BroadcastExchange
Input [1]: [id#210]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=35166]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=35170]
...
PushedFilters: [IsNotNull(name_pcode_cf), IsNotNull(gender), StringStartsWith(name_pcode_cf,D), EqualTo(gender,m), IsNotNull(id)]
...
Condition : ((((isnotnull(name_pcode_cf#545) AND isnotnull(gender#544)) AND StartsWith(name_pcode_cf#545, D)) AND (gender#544 = m)) AND isnotnull(id#540))
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[1, int, true] as bigint), 32) | (cast(input[0, int, true] as bigint) & 4294967295))),false), [plan_id=35173]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[4, int, false] as bigint)),false), [plan_id=35176]
...
Condition : (((isnotnull(production_year#43) AND (cast(production_year#43 as int) >= 1980)) AND (cast(production_year#43 as int) <= 1984)) AND isnotnull(id#39))
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[4, int, false] as bigint), 32) | (cast(input[5, int, true] as bigint) & 4294967295))),false), [plan_id=35179]
...
Input [1]: [id#113]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=35183]
...
Arguments: HashedRelationBroadcastMode(List(input[3, int, true], input[1, int, true], input[0, int, true]),false), [plan_id=35187]
...
Aggregate Attributes [2]: [min#2426, min#2427]
Results [2]: [min#2428, min#2429]

(42) Exchange
Input [2]: [min#2428, min#2429]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=35192]

(43) SortAggregate
Input [2]: [min#2428, min#2429]
Keys: []
Functions [2]: [min(name#541), min(title#40)]
Aggregate Attributes [2]: [min(name#541)#2424, min(title#40)#2425]
Results [2]: [min(name#541)#2424 AS of_person#2417, min(title#40)#2425 AS biography_movie#2418]

(44) AdaptiveSparkPlan
Output [2]: [of_person#2417, biography_movie#2418]
Arguments: isFinalPlan=false
Execution Time: 16.616
