...
(6) BroadcastExchange
Input [1]: [id#23]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=36580]
...
(9) BroadcastExchange
Input [1]: [movie_id#33]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=36584]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, false] as bigint) & 4294967295))),false), [plan_id=36587]
...
PushedFilters: [IsNotNull(role), EqualTo(role,costume designer), IsNotNull(id)]
ReadSchema: struct<id:int,role:string>

(19) Filter
Input [2]: [id#37, role#38]
Condition : ((isnotnull(role#38) AND (role#38 = costume designer)) AND isnotnull(id#37))
...
Input [1]: [id#37]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=36591]
...
Arguments: hashpartitioning(person_id#17, 200), ENSURE_REQUIREMENTS, [plan_id=36596]
...
(28) Exchange
Input [1]: [id#540]
Arguments: hashpartitioning(id#540, 200), ENSURE_REQUIREMENTS, [plan_id=36597]
...
Arguments: hashpartitioning(person_id#17, person_id#17, 200), ENSURE_REQUIREMENTS, [plan_id=36604]
...
(35) Exchange
Input [2]: [person_id#533, name#534]
Arguments: hashpartitioning(person_id#533, person_id#533, 200), ENSURE_REQUIREMENTS, [plan_id=36603]
...
Aggregate Attributes [2]: [min#2511, min#2512]
Results [2]: [min#2513, min#2514]

(40) Exchange
Input [2]: [min#2513, min#2514]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=36611]

(41) SortAggregate
Input [2]: [min#2513, min#2514]
Keys: []
Functions [2]: [min(name#534), min(title#40)]
Aggregate Attributes [2]: [min(name#534)#2509, min(title#40)#2510]
Results [2]: [min(name#534)#2509 AS costume_designer_pseudo#2502, min(title#40)#2510 AS movie_with_costumes#2503]

(42) AdaptiveSparkPlan
Output [2]: [costume_designer_pseudo#2502, movie_with_costumes#2503]
Arguments: isFinalPlan=false
Execution Time: 46.731
