...
(7) BroadcastExchange
Input [1]: [id#37]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=11181]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=11185]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[1, int, true] as bigint)),false), [plan_id=11188]

(16) Scan parquet spark_catalog.imdb_10x.title
Output [3]: [id#39, title#40, production_year#43]
Batched: true
Location: InMemoryFileIndex [file:/path/to/datasets/imdb_10x_parquet/title]
PushedFilters: [IsNotNull(production_year), IsNotNull(title), IsNotNull(id)]
ReadSchema: struct<id:int,title:string,production_year:string>

(17) Filter
Input [3]: [id#39, title#40, production_year#43]
Condition : (((((isnotnull(production_year#43) AND isnotnull(title#40)) AND (cast(production_year#43 as int) >= 2007)) AND (cast(production_year#43 as int) <= 2008)) AND title#40 LIKE %Kung%Fu%Panda%) AND isnotnull(id#39))
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[1, int, true] as bigint), 32) | (cast(input[5, int, true] as bigint) & 4294967295))),false), [plan_id=11191]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[8, int, true] as bigint)),false), [plan_id=11194]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[3, int, true] as bigint) & 4294967295))),false), [plan_id=11198]
...
Arguments: HashedRelationBroadcastMode(List(input[5, int, true], input[0, int, true], input[3, int, true]),false), [plan_id=11202]
...
Condition : (((isnotnull(info#215) AND (info#215 LIKE Japan:%2007% OR info#215 LIKE USA:%2008%)) AND isnotnull(movie_id#213)) AND isnotnull(info_type_id#214))
...
Input [1]: [id#210]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=11206]
...
Arguments: HashedRelationBroadcastMode(List(cast(cast(input[0, string, true] as int) as bigint)),false), [plan_id=11210]
...
Aggregate Attributes [2]: [min#794, min#795]
Results [2]: [min#796, min#797]

(54) Exchange
Input [2]: [min#796, min#797]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=11215]

(55) SortAggregate
Input [2]: [min#796, min#797]
Keys: []
Functions [2]: [min(name#541), min(title#40)]
Aggregate Attributes [2]: [min(name#541)#792, min(title#40)#793]
Results [2]: [min(name#541)#792 AS voicing_actress#785, min(title#40)#793 AS kung_fu_panda#786]

(56) AdaptiveSparkPlan
Output [2]: [voicing_actress#785, kung_fu_panda#786]
Arguments: isFinalPlan=false
Execution Time: 32.486
