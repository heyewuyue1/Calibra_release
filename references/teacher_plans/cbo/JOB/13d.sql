...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=4417]
...
Input [1]: [id#30]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=4421]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[1, int, true] as bigint)),false), [plan_id=4425]
...
Input [1]: [id#293]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=4428]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[1, int, true] as bigint), 32) | (cast(input[2, int, true] as bigint) & 4294967295))),false), [plan_id=4432]
...
Input [1]: [id#210]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=4435]
...
Arguments: hashpartitioning(movie_id#33, movie_id#218, id#39, 200), ENSURE_REQUIREMENTS, [plan_id=4443]
...
(40) Filter
Input [2]: [id#368, info#369]
Condition : ((isnotnull(info#369) AND (info#369 = release dates)) AND isnotnull(id#368))
...
Input [1]: [id#368]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=4438]
...
Input [3]: [movie_id#213, info_type_id#214, id#368]

(45) Exchange
Input [1]: [movie_id#213]
Arguments: hashpartitioning(movie_id#213, movie_id#213, movie_id#213, 200), ENSURE_REQUIREMENTS, [plan_id=4444]
...
Aggregate Attributes [3]: [min#374, min#375, min#376]
Results [3]: [min#377, min#378, min#379]

(50) Exchange
Input [3]: [min#377, min#378, min#379]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=4451]

(51) SortAggregate
Input [3]: [min#377, min#378, min#379]
Keys: []
Functions [3]: [min(name#24), min(info#220), min(title#40)]
Aggregate Attributes [3]: [min(name#24)#370, min(info#220)#371, min(title#40)#372]
Results [3]: [min(name#24)#370 AS producing_company#360, min(info#220)#371 AS rating#361, min(title#40)#372 AS movie#362]

(52) AdaptiveSparkPlan
Output [3]: [producing_company#360, rating#361, movie#362]
Arguments: isFinalPlan=false
Execution Time: 33.677
