...
PushedFilters: [IsNotNull(keyword), EqualTo(keyword,10,000-mile-club), IsNotNull(id)]
ReadSchema: struct<id:int,keyword:string>

(2) Filter
Input [2]: [id#110, keyword#111]
Condition : ((isnotnull(keyword#111) AND (keyword#111 = 10,000-mile-club)) AND isnotnull(id#110))
...
(4) BroadcastExchange
Input [1]: [id#110]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=29692]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=29696]
...
Input [2]: [id#39, title#40]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=29700]
...
Input [2]: [id#113, link#114]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, false] as bigint)),false), [plan_id=29704]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[1, int, true] as bigint)),false), [plan_id=29708]

(25) Scan parquet spark_catalog.imdb_10x.title
Output [2]: [id#1869, title#1870]
Batched: true
Location: InMemoryFileIndex [file:/path/to/datasets/imdb_10x_parquet/title]
PushedFilters: [IsNotNull(id)]
ReadSchema: struct<id:int,title:string>
...
(26) Filter
Input [2]: [id#1869, title#1870]
Condition : isnotnull(id#1869)

(27) BroadcastHashJoin
Left keys [1]: [linked_movie_id#120]
Right keys [1]: [id#1869]
Join type: Inner
Join condition: None

(28) Project
Output [3]: [link#114, title#40, title#1870]
Input [5]: [title#40, linked_movie_id#120, link#114, id#1869, title#1870]

(29) SortAggregate
Input [3]: [link#114, title#40, title#1870]
Keys: []
Functions [3]: [partial_min(link#114), partial_min(title#40), partial_min(title#1870)]
Aggregate Attributes [3]: [min#1885, min#1886, min#1887]
Results [3]: [min#1888, min#1889, min#1890]

(30) Exchange
Input [3]: [min#1888, min#1889, min#1890]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=29713]

(31) SortAggregate
Input [3]: [min#1888, min#1889, min#1890]
Keys: []
Functions [3]: [min(link#114), min(title#40), min(title#1870)]
Aggregate Attributes [3]: [min(link#114)#1881, min(title#40)#1882, min(title#1870)#1883]
Results [3]: [min(link#114)#1881 AS link_type#1861, min(title#40)#1882 AS first_movie#1862, min(title#1870)#1883 AS second_movie#1863]

(32) AdaptiveSparkPlan
Output [3]: [link_type#1861, first_movie#1862, second_movie#1863]
Arguments: isFinalPlan=false
Execution Time: 10.992
