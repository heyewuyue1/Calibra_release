...
(5) BroadcastExchange
Input [2]: [id#110, keyword#111]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, false] as bigint)),false), [plan_id=33545]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=33549]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[2, int, true] as bigint) & 4294967295))),false), [plan_id=33552]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[2, int, true] as bigint)),false), [plan_id=33556]
...
Aggregate Attributes [3]: [min#2285, min#2286, min#2287]
Results [3]: [min#2288, min#2289, min#2290]

(24) Exchange
Input [3]: [min#2288, min#2289, min#2290]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=33561]

(25) SortAggregate
Input [3]: [min#2288, min#2289, min#2290]
Keys: []
Functions [3]: [min(keyword#111), min(name#541), min(title#40)]
Aggregate Attributes [3]: [min(keyword#111)#2282, min(name#541)#2283, min(title#40)#2284]
Results [3]: [min(keyword#111)#2282 AS movie_keyword#2274, min(name#541)#2283 AS actor_name#2275, min(title#40)#2284 AS marvel_movie#2276]

(26) AdaptiveSparkPlan
Output [3]: [movie_keyword#2274, actor_name#2275, marvel_movie#2276]
Arguments: isFinalPlan=false
Execution Time: 15.275
