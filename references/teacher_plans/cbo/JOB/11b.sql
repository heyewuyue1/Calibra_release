      ...
      +- SortAggregate (41)
         +- Project (40)
            +- BroadcastHashJoin Inner BuildRight (39)
               ...
               :     +- Filter (33)
               :        +- Scan parquet spark_catalog.imdb_10x.movie_link (32)
               +- BroadcastExchange (38)
                  +- Filter (37)
                     +- Scan parquet spark_catalog.imdb_10x.link_type (36)
...
(6) BroadcastExchange
Input [1]: [id#110]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=1433]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=1437]

(10) Scan parquet spark_catalog.imdb_10x.title
Output [3]: [id#39, title#40, production_year#43]
Batched: true
Location: InMemoryFileIndex [file:/path/to/datasets/imdb_10x_parquet/title]
PushedFilters: [IsNotNull(production_year), IsNotNull(title), StringContains(title,Money), IsNotNull(id)]
ReadSchema: struct<id:int,title:string,production_year:string>

(11) Filter
Input [3]: [id#39, title#40, production_year#43]
Condition : ((((isnotnull(production_year#43) AND isnotnull(title#40)) AND (cast(production_year#43 as int) = 1998)) AND Contains(title#40, Money)) AND isnotnull(id#39))
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, true] as bigint) & 4294967295))),false), [plan_id=1440]
...
Input [1]: [id#30]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=1443]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[4, int, true] as bigint)),false), [plan_id=1447]
...
Arguments: HashedRelationBroadcastMode(List(input[0, int, true], input[3, int, true], input[1, int, true]),false), [plan_id=1451]

(32) Scan parquet spark_catalog.imdb_10x.movie_link
...
(35) Project
Output [3]: [title#40, name#24, link_type_id#121]
Input [7]: [movie_id#116, id#39, title#40, movie_id#33, name#24, movie_id#119, link_type_id#121]

(36) Scan parquet spark_catalog.imdb_10x.link_type
Output [2]: [id#113, link#114]
Batched: true
Location: InMemoryFileIndex [file:/path/to/datasets/imdb_10x_parquet/link_type]
PushedFilters: [IsNotNull(link), StringContains(link,follows), IsNotNull(id)]
ReadSchema: struct<id:int,link:string>

(37) Filter
Input [2]: [id#113, link#114]
Condition : ((isnotnull(link#114) AND Contains(link#114, follows)) AND isnotnull(id#113))

(38) BroadcastExchange
Input [2]: [id#113, link#114]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, false] as bigint)),false), [plan_id=1455]
...
Input [5]: [title#40, name#24, link_type_id#121, id#113, link#114]

(41) SortAggregate
Input [3]: [name#24, link#114, title#40]
Keys: []
Functions [3]: [partial_min(name#24), partial_min(link#114), partial_min(title#40)]
Aggregate Attributes [3]: [min#150, min#151, min#152]
Results [3]: [min#153, min#154, min#155]

(42) Exchange
Input [3]: [min#153, min#154, min#155]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=1460]

(43) SortAggregate
Input [3]: [min#153, min#154, min#155]
Keys: []
Functions [3]: [min(name#24), min(link#114), min(title#40)]
Aggregate Attributes [3]: [min(name#24)#147, min(link#114)#148, min(title#40)#149]
Results [3]: [min(name#24)#147 AS from_company#139, min(link#114)#148 AS movie_link_type#140, min(title#40)#149 AS sequel_movie#141]

(44) AdaptiveSparkPlan
Output [3]: [from_company#139, movie_link_type#140, sequel_movie#141]
Arguments: isFinalPlan=false
Execution Time: 14.133
