...
(6) BroadcastExchange
Input [1]: [id#110]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=7971]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=7975]
...
Condition : (((isnotnull(episode_nr#48) AND (cast(episode_nr#48 as int) >= 5)) AND (cast(episode_nr#48 as int) < 100)) AND isnotnull(id#39))
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, true] as bigint) & 4294967295))),false), [plan_id=7978]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[4, int, false] as bigint)),false), [plan_id=7981]
...
Arguments: HashedRelationBroadcastMode(List(input[3, int, true], input[0, int, true], input[1, int, true]),false), [plan_id=7985]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[1, int, true] as bigint)),false), [plan_id=7989]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[1, int, true] as bigint), 32) | (cast(input[2, int, false] as bigint) & 4294967295))),false), [plan_id=7992]
...
Aggregate Attributes [2]: [min#604, min#605]
Results [2]: [min#606, min#607]

(39) Exchange
Input [2]: [min#606, min#607]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=7997]

(40) SortAggregate
Input [2]: [min#606, min#607]
Keys: []
Functions [2]: [min(name#534), min(title#40)]
Aggregate Attributes [2]: [min(name#534)#602, min(title#40)#603]
Results [2]: [min(name#534)#602 AS cool_actor_pseudonym#595, min(title#40)#603 AS series_named_after_char#596]

(41) AdaptiveSparkPlan
Output [2]: [cool_actor_pseudonym#595, series_named_after_char#596]
Arguments: isFinalPlan=false
Execution Time: 28.143
