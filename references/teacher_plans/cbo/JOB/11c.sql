...
(6) BroadcastExchange
Input [1]: [id#110]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=1747]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=1751]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, true] as bigint) & 4294967295))),false), [plan_id=1754]
...
Input [1]: [id#30]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=1757]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[4, int, true] as bigint)),false), [plan_id=1761]
...
PushedFilters: [IsNotNull(country_code), Not(EqualTo(country_code,[pl])), Or(StringStartsWith(name,20th Century Fox),StringStartsWith(name,Twentieth Century Fox)), IsNotNull(id)]
ReadSchema: struct<id:int,name:string,country_code:string>

(26) Filter
Input [3]: [id#23, name#24, country_code#25]
Condition : (((isnotnull(country_code#25) AND NOT (country_code#25 = [pl])) AND (StartsWith(name#24, 20th Century Fox) OR StartsWith(name#24, Twentieth Century Fox))) AND isnotnull(id#23))
...
Arguments: HashedRelationBroadcastMode(List(input[0, int, true], input[3, int, true], input[1, int, true]),false), [plan_id=1765]
...
Input [1]: [id#113]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, false] as bigint)),false), [plan_id=1769]
...
Aggregate Attributes [3]: [min#171, min#172, min#173]
Results [3]: [min#174, min#175, min#176]

(41) Exchange
Input [3]: [min#174, min#175, min#176]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=1774]

(42) SortAggregate
Input [3]: [min#174, min#175, min#176]
Keys: []
Functions [3]: [min(name#24), min(note#36), min(title#40)]
Aggregate Attributes [3]: [min(name#24)#168, min(note#36)#169, min(title#40)#170]
Results [3]: [min(name#24)#168 AS from_company#160, min(note#36)#169 AS production_note#161, min(title#40)#170 AS movie_based_on_book#162]

(43) AdaptiveSparkPlan
Output [3]: [from_company#160, production_note#161, movie_based_on_book#162]
Arguments: isFinalPlan=false
Execution Time: 17.702
