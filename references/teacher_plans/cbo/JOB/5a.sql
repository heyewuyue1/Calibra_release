...
PushedFilters: [In(info, [Denish,Denmark,German,Germany,Norway,Norwegian,Sweden,Swedish]), IsNotNull(movie_id), IsNotNull(info_type_id)]
ReadSchema: struct<movie_id:int,info_type_id:int,info:string>

(2) Filter
Input [3]: [movie_id#213, info_type_id#214, info#215]
Condition : ((info#215 IN (Sweden,Norway,Germany,Denmark,Swedish,Denish,Norwegian,German) AND isnotnull(movie_id#213)) AND isnotnull(info_type_id#214))
...
Arguments: HashedRelationBroadcastMode(List(cast(input[1, int, true] as bigint)),false), [plan_id=32930]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=32934]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, true] as bigint) & 4294967295))),false), [plan_id=32937]
...
PushedFilters: [IsNotNull(note), StringContains(note,(theatrical)), StringContains(note,(France)), IsNotNull(company_type_id), IsNotNull(movie_id)]
ReadSchema: struct<movie_id:int,company_type_id:int,note:string>

(16) Filter
Input [3]: [movie_id#33, company_type_id#35, note#36]
Condition : ((((isnotnull(note#36) AND Contains(note#36, (theatrical))) AND Contains(note#36, (France))) AND isnotnull(company_type_id#35)) AND isnotnull(movie_id#33))
...
Input [1]: [id#30]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=32941]
...
Aggregate Attributes [1]: [min#2242]
Results [1]: [min#2243]

(27) Exchange
Input [1]: [min#2243]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=32946]

(28) SortAggregate
Input [1]: [min#2243]
Keys: []
Functions [1]: [min(title#40)]
Aggregate Attributes [1]: [min(title#40)#2241]
Results [1]: [min(title#40)#2241 AS typical_european_movie#2235]

(29) AdaptiveSparkPlan
Output [1]: [typical_european_movie#2235]
Arguments: isFinalPlan=false
Execution Time: 19.801
