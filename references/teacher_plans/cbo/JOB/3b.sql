...
PushedFilters: [IsNotNull(info), EqualTo(info,Bulgaria), IsNotNull(movie_id)]
ReadSchema: struct<movie_id:int,info:string>

(2) Filter
Input [2]: [movie_id#213, info#215]
Condition : ((isnotnull(info#215) AND (info#215 = Bulgaria)) AND isnotnull(movie_id#213))
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=31913]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, true] as bigint) & 4294967295))),false), [plan_id=31916]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[1, int, true] as bigint)),false), [plan_id=31920]
...
Aggregate Attributes [1]: [min#2165]
Results [1]: [min#2166]

(21) Exchange
Input [1]: [min#2166]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=31925]

(22) SortAggregate
Input [1]: [min#2166]
Keys: []
Functions [1]: [min(title#40)]
Aggregate Attributes [1]: [min(title#40)#2164]
Results [1]: [min(title#40)#2164 AS movie_title#2158]

(23) AdaptiveSparkPlan
Output [1]: [movie_title#2158]
Arguments: isFinalPlan=false
Execution Time: 16.986
