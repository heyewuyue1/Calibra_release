...
(6) BroadcastExchange
Input [1]: [id#110]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=8799]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=8803]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, false] as bigint) & 4294967295))),false), [plan_id=8806]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[3, int, false] as bigint)),false), [plan_id=8809]
...
Arguments: HashedRelationBroadcastMode(List(input[2, int, true], input[0, int, true], input[1, int, true]),false), [plan_id=8813]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=8817]
...
PushedFilters: [IsNotNull(name), StringStartsWith(name,X), IsNotNull(id)]
ReadSchema: struct<id:int,name:string>

(29) Filter
Input [2]: [id#540, name#541]
Condition : ((isnotnull(name#541) AND StartsWith(name#541, X)) AND isnotnull(id#540))
...
Aggregate Attributes [1]: [min#651]
Results [1]: [min#652]

(33) Exchange
Input [1]: [min#652]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=8822]

(34) SortAggregate
Input [1]: [min#652]
Keys: []
Functions [1]: [min(name#541)]
Aggregate Attributes [1]: [min(name#541)#649]
Results [2]: [min(name#541)#649 AS member_in_charnamed_movie#642, min(name#541)#649 AS a1#643]

(35) AdaptiveSparkPlan
Output [2]: [member_in_charnamed_movie#642, a1#643]
Arguments: isFinalPlan=false
Execution Time: 67.257
