...
(6) BroadcastExchange
Input [1]: [id#110]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=22099]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=22103]
...
Input [1]: [id#30]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=22106]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[2, int, true] as bigint)),false), [plan_id=22110]
...
Arguments: HashedRelationBroadcastMode(List(input[1, int, true], input[0, int, true], input[0, int, true], input[1, int, true], input[0, int, true], input[1, int, true]),false), [plan_id=22123]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=22113]
...
Condition : ((isnotnull(production_year#43) AND (cast(production_year#43 as int) = 1998)) AND isnotnull(id#39))
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, true] as bigint) & 4294967295))),false), [plan_id=22116]
...
Input [2]: [id#113, link#114]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, false] as bigint)),false), [plan_id=22119]
...
Arguments: HashedRelationBroadcastMode(List(input[1, int, true], input[3, int, true], input[0, int, true], input[6, int, true], input[4, int, true]),false), [plan_id=22133]
...
(49) Filter
Input [2]: [id#1424, kind#1425]
Condition : ((isnotnull(kind#1425) AND (kind#1425 = complete)) AND isnotnull(id#1424))
...
Input [1]: [id#1424]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=22125]

(52) BroadcastHashJoin
Left keys [1]: [status_id#929]
Right keys [1]: [id#1424]
Join type: Inner
Join condition: None

(53) Project
Output [2]: [movie_id#927, subject_id#928]
Input [4]: [movie_id#927, subject_id#928, status_id#929, id#1424]
...
Input [1]: [id#930]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=22129]
...
Aggregate Attributes [3]: [min#1430, min#1431, min#1432]
Results [3]: [min#1433, min#1434, min#1435]

(63) Exchange
Input [3]: [min#1433, min#1434, min#1435]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=22138]

(64) SortAggregate
Input [3]: [min#1433, min#1434, min#1435]
Keys: []
Functions [3]: [min(name#24), min(link#114), min(title#40)]
Aggregate Attributes [3]: [min(name#24)#1427, min(link#114)#1428, min(title#40)#1429]
Results [3]: [min(name#24)#1427 AS producing_company#1416, min(link#114)#1428 AS link_type#1417, min(title#40)#1429 AS complete_western_sequel#1418]

(65) AdaptiveSparkPlan
Output [3]: [producing_company#1416, link_type#1417, complete_western_sequel#1418]
Arguments: isFinalPlan=false
Execution Time: 27.144
