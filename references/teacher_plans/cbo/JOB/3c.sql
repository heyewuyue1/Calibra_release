...
PushedFilters: [In(info, [American,Denish,Denmark,German,Germany,Norway,Norwegian,Sweden,Swedish,USA]), IsNotNull(movie_id)]
ReadSchema: struct<movie_id:int,info:string>

(2) Filter
Input [2]: [movie_id#213, info#215]
Condition : (info#215 IN (Sweden,Norway,Germany,Denmark,Swedish,Denish,Norwegian,German,USA,American) AND isnotnull(movie_id#213))
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=32078]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, true] as bigint) & 4294967295))),false), [plan_id=32081]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[1, int, true] as bigint)),false), [plan_id=32085]
...
Aggregate Attributes [1]: [min#2178]
Results [1]: [min#2179]

(21) Exchange
Input [1]: [min#2179]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=32090]

(22) SortAggregate
Input [1]: [min#2179]
Keys: []
Functions [1]: [min(title#40)]
Aggregate Attributes [1]: [min(title#40)#2177]
Results [1]: [min(title#40)#2177 AS movie_title#2171]

(23) AdaptiveSparkPlan
Output [1]: [movie_title#2171]
Arguments: isFinalPlan=false
Execution Time: 108.073
