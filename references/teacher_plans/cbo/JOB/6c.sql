...
(5) BroadcastExchange
Input [2]: [id#110, keyword#111]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, false] as bigint)),false), [plan_id=33955]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=33959]
...
Condition : ((isnotnull(production_year#43) AND (cast(production_year#43 as int) > 2014)) AND isnotnull(id#39))
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[2, int, true] as bigint) & 4294967295))),false), [plan_id=33962]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[2, int, true] as bigint)),false), [plan_id=33966]
...
Aggregate Attributes [3]: [min#2327, min#2328, min#2329]
Results [3]: [min#2330, min#2331, min#2332]

(24) Exchange
Input [3]: [min#2330, min#2331, min#2332]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=33971]

(25) SortAggregate
Input [3]: [min#2330, min#2331, min#2332]
Keys: []
Functions [3]: [min(keyword#111), min(name#541), min(title#40)]
Aggregate Attributes [3]: [min(keyword#111)#2324, min(name#541)#2325, min(title#40)#2326]
Results [3]: [min(keyword#111)#2324 AS movie_keyword#2316, min(name#541)#2325 AS actor_name#2317, min(title#40)#2326 AS marvel_movie#2318]

(26) AdaptiveSparkPlan
Output [3]: [movie_keyword#2316, actor_name#2317, marvel_movie#2318]
Arguments: isFinalPlan=false
Execution Time: 15.075
