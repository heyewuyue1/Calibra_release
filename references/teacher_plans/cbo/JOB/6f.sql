...
(5) BroadcastExchange
Input [2]: [id#110, keyword#111]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, false] as bigint)),false), [plan_id=34570]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=34574]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[2, int, true] as bigint) & 4294967295))),false), [plan_id=34577]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[2, int, true] as bigint)),false), [plan_id=34581]
...
Aggregate Attributes [3]: [min#2390, min#2391, min#2392]
Results [3]: [min#2393, min#2394, min#2395]

(24) Exchange
Input [3]: [min#2393, min#2394, min#2395]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=34586]

(25) SortAggregate
Input [3]: [min#2393, min#2394, min#2395]
Keys: []
Functions [3]: [min(keyword#111), min(name#541), min(title#40)]
Aggregate Attributes [3]: [min(keyword#111)#2387, min(name#541)#2388, min(title#40)#2389]
Results [3]: [min(keyword#111)#2387 AS movie_keyword#2379, min(name#541)#2388 AS actor_name#2380, min(title#40)#2389 AS hero_movie#2381]

(26) AdaptiveSparkPlan
Output [3]: [movie_keyword#2379, actor_name#2380, hero_movie#2381]
Arguments: isFinalPlan=false
Execution Time: 26.925
