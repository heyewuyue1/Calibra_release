...
PushedFilters: [In(info, [American,Denish,Denmark,German,Germany,Norway,Norwegian,Sweden,Swedish,USA]), IsNotNull(movie_id), IsNotNull(info_type_id)]
ReadSchema: struct<movie_id:int,info_type_id:int,info:string>

(2) Filter
Input [3]: [movie_id#213, info_type_id#214, info#215]
Condition : ((info#215 IN (Sweden,Norway,Germany,Denmark,Swedish,Denish,Norwegian,German,USA,American) AND isnotnull(movie_id#213)) AND isnotnull(info_type_id#214))
...
Arguments: HashedRelationBroadcastMode(List(cast(input[1, int, true] as bigint)),false), [plan_id=33340]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=33344]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, true] as bigint) & 4294967295))),false), [plan_id=33347]
...
PushedFilters: [IsNotNull(note), Not(StringContains(note,(TV))), StringContains(note,(USA)), IsNotNull(company_type_id), IsNotNull(movie_id)]
ReadSchema: struct<movie_id:int,company_type_id:int,note:string>

(16) Filter
Input [3]: [movie_id#33, company_type_id#35, note#36]
Condition : ((((isnotnull(note#36) AND NOT Contains(note#36, (TV))) AND Contains(note#36, (USA))) AND isnotnull(company_type_id#35)) AND isnotnull(movie_id#33))
...
Input [1]: [id#30]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=33351]
...
Aggregate Attributes [1]: [min#2268]
Results [1]: [min#2269]

(27) Exchange
Input [1]: [min#2269]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=33356]

(28) SortAggregate
Input [1]: [min#2269]
Keys: []
Functions [1]: [min(title#40)]
Aggregate Attributes [1]: [min(title#40)#2267]
Results [1]: [min(title#40)#2267 AS american_movie#2261]

(29) AdaptiveSparkPlan
Output [1]: [american_movie#2261]
Arguments: isFinalPlan=false
Execution Time: 87.606
