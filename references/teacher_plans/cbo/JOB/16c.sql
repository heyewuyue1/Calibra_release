...
(6) BroadcastExchange
Input [1]: [id#110]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=7665]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=7669]
...
Condition : ((isnotnull(episode_nr#48) AND (cast(episode_nr#48 as int) < 100)) AND isnotnull(id#39))
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, true] as bigint) & 4294967295))),false), [plan_id=7672]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[4, int, false] as bigint)),false), [plan_id=7675]
...
Arguments: HashedRelationBroadcastMode(List(input[3, int, true], input[0, int, true], input[1, int, true]),false), [plan_id=7679]
...
Arguments: HashedRelationBroadcastMode(List(cast(input[1, int, true] as bigint)),false), [plan_id=7683]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[1, int, true] as bigint), 32) | (cast(input[2, int, false] as bigint) & 4294967295))),false), [plan_id=7686]
...
Aggregate Attributes [2]: [min#587, min#588]
Results [2]: [min#589, min#590]

(39) Exchange
Input [2]: [min#589, min#590]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=7691]

(40) SortAggregate
Input [2]: [min#589, min#590]
Keys: []
Functions [2]: [min(name#534), min(title#40)]
Aggregate Attributes [2]: [min(name#534)#585, min(title#40)#586]
Results [2]: [min(name#534)#585 AS cool_actor_pseudonym#578, min(title#40)#586 AS series_named_after_char#579]

(41) AdaptiveSparkPlan
Output [2]: [cool_actor_pseudonym#578, series_named_after_char#579]
Arguments: isFinalPlan=false
Execution Time: 27.993
