...
(4) BroadcastExchange
Input [1]: [id#210]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=5352]
...
PushedFilters: [In(info, [American,Danish,Denmark,German,Germany,Norway,Norwegian,Sweden,Swedish,USA]), IsNotNull(info_type_id), IsNotNull(movie_id)]
ReadSchema: struct<movie_id:int,info_type_id:int,info:string>

(6) Filter
Input [3]: [movie_id#213, info_type_id#214, info#215]
Condition : ((info#215 IN (Sweden,Norway,Germany,Denmark,Swedish,Danish,Norwegian,German,USA,American) AND isnotnull(info_type_id#214)) AND isnotnull(movie_id#213))
...
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=5356]
...
Input [1]: [id#293]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=5359]
...
Arguments: HashedRelationBroadcastMode(List((shiftleft(cast(input[0, int, true] as bigint), 32) | (cast(input[1, int, true] as bigint) & 4294967295))),false), [plan_id=5363]
...
(26) Filter
Input [2]: [id#431, info#432]
Condition : ((isnotnull(info#432) AND (info#432 = rating)) AND isnotnull(id#431))
...
Input [1]: [id#431]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=5366]
...
Input [7]: [movie_id#213, id#39, title#40, movie_id#218, info_type_id#219, info#220, id#431]
...
PushedFilters: [IsNotNull(keyword), In(keyword, [blood,murder,murder-in-title,violence]), IsNotNull(id)]
ReadSchema: struct<id:int,keyword:string>

(34) Filter
Input [2]: [id#110, keyword#111]
Condition : ((isnotnull(keyword#111) AND keyword#111 IN (murder,murder-in-title,blood,violence)) AND isnotnull(id#110))
...
Input [1]: [id#110]
Arguments: HashedRelationBroadcastMode(List(cast(input[0, int, true] as bigint)),false), [plan_id=5369]
...
Arguments: HashedRelationBroadcastMode(List(input[0, int, true], input[0, int, true], input[0, int, true]),false), [plan_id=5373]
...
Aggregate Attributes [2]: [min#436, min#437]
Results [2]: [min#438, min#439]

(43) Exchange
Input [2]: [min#438, min#439]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=5378]

(44) SortAggregate
Input [2]: [min#438, min#439]
Keys: []
Functions [2]: [min(info#220), min(title#40)]
Aggregate Attributes [2]: [min(info#220)#434, min(title#40)#435]
Results [2]: [min(info#220)#434 AS rating#424, min(title#40)#435 AS north_european_dark_production#425]

(45) AdaptiveSparkPlan
Output [2]: [rating#424, north_european_dark_production#425]
Arguments: isFinalPlan=false
Execution Time: 28.329
