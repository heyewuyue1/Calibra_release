26/04/18 03:11:46 WARN NativeCodeLoader: Unable to load native-hadoop library for your platform... using builtin-java classes where applicable
26/04/18 03:11:46 WARN SparkConf: Note that spark.local.dir will be overridden by the value set by the cluster manager (via SPARK_LOCAL_DIRS in mesos/standalone/kubernetes and LOCAL_DIRS in YARN).
26/04/18 03:11:49 WARN HiveConf: HiveConf of name hive.stats.jdbc.timeout does not exist
26/04/18 03:11:49 WARN HiveConf: HiveConf of name hive.stats.retries.wait does not exist
26/04/18 03:11:52 WARN ObjectStore: Version information not found in metastore. hive.metastore.schema.verification is not enabled so recording the schema version 2.3.0
26/04/18 03:11:52 WARN ObjectStore: setMetaStoreSchemaVersion called but recording version is disabled: version = 2.3.0, comment = Set by MetaStore user@YOUR_HOST
26/04/18 03:11:53 WARN ObjectStore: Failed to get database global_temp, returning NoSuchObjectException
Spark Web UI available at http://YOUR_SPARK_MASTER_HOST:4040
Spark master: spark://YOUR_SPARK_MASTER_HOST:7077, Application Id: app-20260418031147-7150
== Physical Plan ==
...
PushedFilters: [IsNotNull(role_id), EqualTo(role_id,1), IsNotNull(movie_id), IsNotNull(person_role_id)]
ReadSchema: struct<movie_id:int,person_role_id:string,role_id:int>

(2) Filter
Input [3]: [movie_id#7, person_role_id#8, role_id#11]
Condition : (((isnotnull(role_id#11) AND (role_id#11 = 1)) AND isnotnull(movie_id#7)) AND isnotnull(person_role_id#8))
...
PushedFilters: [IsNotNull(kind_id), EqualTo(kind_id,3), IsNotNull(id)]
...
Condition : ((isnotnull(kind_id#15) AND (kind_id#15 = 3)) AND isnotnull(id#12))
...
Keys: []
Functions [3]: [partial_min(title#13), partial_max(production_year#16), partial_min(name#25)]
Aggregate Attributes [3]: [min#40, max#41, min#42]
Results [3]: [min#43, max#44, min#45]

(22) Exchange
Input [3]: [min#43, max#44, min#45]
Arguments: SinglePartition, ENSURE_REQUIREMENTS, [plan_id=123]

(23) SortAggregate
Input [3]: [min#43, max#44, min#45]
Keys: []
Functions [3]: [min(title#13), max(production_year#16), min(name#25)]
Aggregate Attributes [3]: [min(title#13)#34, max(production_year#16)#35, min(name#25)#36]
Results [3]: [min(title#13)#34 AS min(title)#37, max(production_year#16)#35 AS max(production_year)#38, min(name#25)#36 AS min(name)#39]

(24) AdaptiveSparkPlan
Output [3]: [min(title)#37, max(production_year)#38, min(name)#39]
