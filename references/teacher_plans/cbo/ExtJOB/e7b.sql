26/04/18 03:19:42 WARN NativeCodeLoader: Unable to load native-hadoop library for your platform... using builtin-java classes where applicable
26/04/18 03:19:42 WARN SparkConf: Note that spark.local.dir will be overridden by the value set by the cluster manager (via SPARK_LOCAL_DIRS in mesos/standalone/kubernetes and LOCAL_DIRS in YARN).
26/04/18 03:19:45 WARN HiveConf: HiveConf of name hive.stats.jdbc.timeout does not exist
26/04/18 03:19:45 WARN HiveConf: HiveConf of name hive.stats.retries.wait does not exist
26/04/18 03:19:48 WARN ObjectStore: Version information not found in metastore. hive.metastore.schema.verification is not enabled so recording the schema version 2.3.0
26/04/18 03:19:48 WARN ObjectStore: setMetaStoreSchemaVersion called but recording version is disabled: version = 2.3.0, comment = Set by MetaStore user@YOUR_HOST
26/04/18 03:19:49 WARN ObjectStore: Failed to get database global_temp, returning NoSuchObjectException
Spark Web UI available at http://YOUR_SPARK_MASTER_HOST:4040
Spark master: spark://YOUR_SPARK_MASTER_HOST:7077, Application Id: app-20260418031943-7174
== Physical Plan ==
...
PushedFilters: [IsNotNull(production_year), Or(Or(StringContains(title,Freddy),StringContains(title,Jason)),StringStartsWith(title,Saw)), IsNotNull(id), IsNotNull(kind_id)]
...
Condition : ((((isnotnull(production_year#50) AND (cast(production_year#50 as int) > 2008)) AND ((Contains(title#47, Freddy) OR Contains(title#47, Jason)) OR StartsWith(title#47, Saw))) AND isnotnull(id#46)) AND isnotnull(kind_id#49))
...
(26) Filter
Input [2]: [id#16, name#17]
Condition : ((isnotnull(name#17) AND StartsWith(name#17, Lionsgate)) AND isnotnull(id#16))
...
Results [4]: [min(info#36)#73 AS movie_budget#0, min(info#41)#74 AS movie_votes#1, min(kind#59)#75 AS movie_type#2, min(title#47)#76 AS violent_liongate_movie#3]

(59) AdaptiveSparkPlan
Output [4]: [movie_budget#0, movie_votes#1, movie_type#2, violent_liongate_movie#3]
Arguments: isFinalPlan=false


Time taken: 4.264 seconds, Fetched 1 row(s)
