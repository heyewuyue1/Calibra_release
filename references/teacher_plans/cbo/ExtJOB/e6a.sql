26/04/18 03:17:25 WARN NativeCodeLoader: Unable to load native-hadoop library for your platform... using builtin-java classes where applicable
26/04/18 03:17:26 WARN SparkConf: Note that spark.local.dir will be overridden by the value set by the cluster manager (via SPARK_LOCAL_DIRS in mesos/standalone/kubernetes and LOCAL_DIRS in YARN).
26/04/18 03:17:29 WARN HiveConf: HiveConf of name hive.stats.jdbc.timeout does not exist
26/04/18 03:17:29 WARN HiveConf: HiveConf of name hive.stats.retries.wait does not exist
26/04/18 03:17:31 WARN ObjectStore: Version information not found in metastore. hive.metastore.schema.verification is not enabled so recording the schema version 2.3.0
26/04/18 03:17:31 WARN ObjectStore: setMetaStoreSchemaVersion called but recording version is disabled: version = 2.3.0, comment = Set by MetaStore user@YOUR_HOST
26/04/18 03:17:33 WARN ObjectStore: Failed to get database global_temp, returning NoSuchObjectException
Spark Web UI available at http://YOUR_SPARK_MASTER_HOST:4040
Spark master: spark://YOUR_SPARK_MASTER_HOST:7077, Application Id: app-20260418031727-7168
== Physical Plan ==
...
PushedFilters: [IsNotNull(info), GreaterThan(info,2.0), IsNotNull(movie_id)]
ReadSchema: struct<movie_id:int,info:string>

(2) Filter
Input [2]: [movie_id#8, info#10]
Condition : ((isnotnull(info#10) AND (info#10 > 2.0)) AND isnotnull(movie_id#8))
...
Condition : ((isnotnull(production_year#19) AND (cast(production_year#19 as int) > 2005)) AND isnotnull(id#15))
