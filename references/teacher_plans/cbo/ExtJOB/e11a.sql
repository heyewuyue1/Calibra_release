26/04/18 03:07:47 WARN NativeCodeLoader: Unable to load native-hadoop library for your platform... using builtin-java classes where applicable
26/04/18 03:07:48 WARN SparkConf: Note that spark.local.dir will be overridden by the value set by the cluster manager (via SPARK_LOCAL_DIRS in mesos/standalone/kubernetes and LOCAL_DIRS in YARN).
26/04/18 03:07:51 WARN HiveConf: HiveConf of name hive.stats.jdbc.timeout does not exist
26/04/18 03:07:51 WARN HiveConf: HiveConf of name hive.stats.retries.wait does not exist
26/04/18 03:07:53 WARN ObjectStore: Version information not found in metastore. hive.metastore.schema.verification is not enabled so recording the schema version 2.3.0
26/04/18 03:07:53 WARN ObjectStore: setMetaStoreSchemaVersion called but recording version is disabled: version = 2.3.0, comment = Set by MetaStore user@YOUR_HOST
26/04/18 03:07:55 WARN ObjectStore: Failed to get database global_temp, returning NoSuchObjectException
Spark Web UI available at http://YOUR_SPARK_MASTER_HOST:4040
Spark master: spark://YOUR_SPARK_MASTER_HOST:7077, Application Id: app-20260418030749-7140
== Physical Plan ==
...
Condition : ((((((Contains(lower(info#44), uk) OR Contains(lower(info#44), spain)) OR Contains(lower(info#44), germany)) OR (Contains(lower(info#44), italy) OR Contains(lower(info#44), croatia))) OR ((Contains(lower(info#44), algeria) OR Contains(lower(info#44), south)) OR (Contains(lower(info#44), austria) OR Contains(lower(info#44), hungary)))) AND isnotnull(person_id#42)) AND isnotnull(info_type_id#43))
...
Condition : (((isnotnull(info#23) AND Contains(lower(info#23), france)) AND isnotnull(info_type_id#22)) AND isnotnull(movie_id#21))
...
Time taken: 1.911 seconds, Fetched 1 row(s)
