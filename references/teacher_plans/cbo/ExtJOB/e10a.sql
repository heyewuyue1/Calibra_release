26/04/18 03:06:00 WARN NativeCodeLoader: Unable to load native-hadoop library for your platform... using builtin-java classes where applicable
26/04/18 03:06:01 WARN SparkConf: Note that spark.local.dir will be overridden by the value set by the cluster manager (via SPARK_LOCAL_DIRS in mesos/standalone/kubernetes and LOCAL_DIRS in YARN).
26/04/18 03:06:04 WARN HiveConf: HiveConf of name hive.stats.jdbc.timeout does not exist
26/04/18 03:06:04 WARN HiveConf: HiveConf of name hive.stats.retries.wait does not exist
26/04/18 03:06:06 WARN ObjectStore: Version information not found in metastore. hive.metastore.schema.verification is not enabled so recording the schema version 2.3.0
26/04/18 03:06:06 WARN ObjectStore: setMetaStoreSchemaVersion called but recording version is disabled: version = 2.3.0, comment = Set by MetaStore user@YOUR_HOST
26/04/18 03:06:07 WARN ObjectStore: Failed to get database global_temp, returning NoSuchObjectException
Spark Web UI available at http://YOUR_SPARK_MASTER_HOST:4040
Spark master: spark://YOUR_SPARK_MASTER_HOST:7077, Application Id: app-20260418030602-7136
== Physical Plan ==
...
Condition : ((((StartsWith(lower(info#10), 8) OR StartsWith(lower(info#10), 9)) OR StartsWith(lower(info#10), 10)) AND isnotnull(info_type_id#9)) AND isnotnull(movie_id#8))
...
Condition : (((isnotnull(info#43) AND Contains(lower(info#43), india)) AND isnotnull(person_id#41)) AND isnotnull(info_type_id#42))
