26/04/18 03:21:36 WARN NativeCodeLoader: Unable to load native-hadoop library for your platform... using builtin-java classes where applicable
26/04/18 03:21:36 WARN SparkConf: Note that spark.local.dir will be overridden by the value set by the cluster manager (via SPARK_LOCAL_DIRS in mesos/standalone/kubernetes and LOCAL_DIRS in YARN).
26/04/18 03:21:39 WARN HiveConf: HiveConf of name hive.stats.jdbc.timeout does not exist
26/04/18 03:21:39 WARN HiveConf: HiveConf of name hive.stats.retries.wait does not exist
26/04/18 03:21:41 WARN ObjectStore: Version information not found in metastore. hive.metastore.schema.verification is not enabled so recording the schema version 2.3.0
26/04/18 03:21:41 WARN ObjectStore: setMetaStoreSchemaVersion called but recording version is disabled: version = 2.3.0, comment = Set by MetaStore user@YOUR_HOST
26/04/18 03:21:43 WARN ObjectStore: Failed to get database global_temp, returning NoSuchObjectException
Spark Web UI available at http://YOUR_SPARK_MASTER_HOST:4040
Spark master: spark://YOUR_SPARK_MASTER_HOST:7077, Application Id: app-20260418032137-7180
== Physical Plan ==
...
Condition : ((((isnotnull(info#43) AND Contains(lower(info#43), sci)) AND isnotnull(movie_id#41)) AND (info_type_id#42 = 3)) AND isnotnull(info_type_id#42))
...
PushedFilters: [IsNotNull(info), IsNotNull(info_type_id), IsNotNull(person_id)]
ReadSchema: struct<person_id:int,info_type_id:int,info:string>

(28) Filter
Input [3]: [person_id#6, info_type_id#7, info#8]
Condition : (((isnotnull(info#8) AND Contains(lower(info#8), 199)) AND isnotnull(info_type_id#7)) AND isnotnull(person_id#6))
