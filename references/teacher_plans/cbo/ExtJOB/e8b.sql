26/04/18 03:21:06 WARN NativeCodeLoader: Unable to load native-hadoop library for your platform... using builtin-java classes where applicable
26/04/18 03:21:07 WARN SparkConf: Note that spark.local.dir will be overridden by the value set by the cluster manager (via SPARK_LOCAL_DIRS in mesos/standalone/kubernetes and LOCAL_DIRS in YARN).
26/04/18 03:21:10 WARN HiveConf: HiveConf of name hive.stats.jdbc.timeout does not exist
26/04/18 03:21:10 WARN HiveConf: HiveConf of name hive.stats.retries.wait does not exist
26/04/18 03:21:12 WARN ObjectStore: Version information not found in metastore. hive.metastore.schema.verification is not enabled so recording the schema version 2.3.0
26/04/18 03:21:12 WARN ObjectStore: setMetaStoreSchemaVersion called but recording version is disabled: version = 2.3.0, comment = Set by MetaStore user@YOUR_HOST
26/04/18 03:21:14 WARN ObjectStore: Failed to get database global_temp, returning NoSuchObjectException
Spark Web UI available at http://YOUR_SPARK_MASTER_HOST:4040
Spark master: spark://YOUR_SPARK_MASTER_HOST:7077, Application Id: app-20260418032108-7178
== Physical Plan ==
...
Condition : ((((isnotnull(info#29) AND Contains(lower(info#29), action)) AND isnotnull(movie_id#27)) AND (info_type_id#28 = 3)) AND isnotnull(info_type_id#28))
...
Condition : ((isnotnull(keyword#22) AND Contains(lower(keyword#22), fight)) AND isnotnull(id#21))
...
Time taken: 1.517 seconds, Fetched 1 row(s)
