...
PushedFilters: [IsNotNull(p_size), IsNotNull(p_type), EqualTo(p_size,3), StringEndsWith(p_type,TIN), IsNotNull(p_partkey)]
...
Condition : ((((isnotnull(p_size#204) AND isnotnull(p_type#203)) AND (p_size#204 = 3)) AND EndsWith(p_type#203, TIN)) AND isnotnull(p_partkey#199L))
...
Arguments: hashpartitioning(p_partkey#199L, 200), ENSURE_REQUIREMENTS, [plan_id=5330]
...
Output [2]: [n_nationkey#505L, n_regionkey#507L]
Batched: true
...
(7) Filter
Input [2]: [n_nationkey#505L, n_regionkey#507L]
Condition : (isnotnull(n_nationkey#505L) AND isnotnull(n_regionkey#507L))

(8) Scan parquet spark_catalog.tpch_sf100.region
Output [2]: [r_regionkey#509L, r_name#510]
Batched: true
...
(9) Filter
Input [2]: [r_regionkey#509L, r_name#510]
Condition : ((isnotnull(r_name#510) AND (r_name#510 = AMERICA)) AND isnotnull(r_regionkey#509L))

(10) Project
Output [1]: [r_regionkey#509L]
Input [2]: [r_regionkey#509L, r_name#510]

(11) BroadcastExchange
Input [1]: [r_regionkey#509L]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=5308]

(12) BroadcastHashJoin
Left keys [1]: [n_regionkey#507L]
Right keys [1]: [r_regionkey#509L]
Join type: Inner
Join condition: None

(13) Project
Output [1]: [n_nationkey#505L]
Input [3]: [n_nationkey#505L, n_regionkey#507L, r_regionkey#509L]

(14) BroadcastExchange
Input [1]: [n_nationkey#505L]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=5312]

(15) Scan parquet spark_catalog.tpch_sf100.supplier
Output [2]: [s_suppkey#498L, s_nationkey#501L]
Batched: true
...
(16) Filter
Input [2]: [s_suppkey#498L, s_nationkey#501L]
Condition : (isnotnull(s_suppkey#498L) AND isnotnull(s_nationkey#501L))

(17) BroadcastHashJoin
Left keys [1]: [n_nationkey#505L]
Right keys [1]: [s_nationkey#501L]
Join type: Inner
Join condition: None

(18) Project
Output [1]: [s_suppkey#498L]
Input [3]: [n_nationkey#505L, s_suppkey#498L, s_nationkey#501L]

(19) Exchange
Input [1]: [s_suppkey#498L]
Arguments: hashpartitioning(s_suppkey#498L, 200), ENSURE_REQUIREMENTS, [plan_id=5317]

(20) Sort
Input [1]: [s_suppkey#498L]
Arguments: [s_suppkey#498L ASC NULLS FIRST], false, 0

(21) Scan parquet spark_catalog.tpch_sf100.partsupp
Output [3]: [ps_partkey#493L, ps_suppkey#494L, ps_supplycost#496]
Batched: true
...
(22) Filter
Input [3]: [ps_partkey#493L, ps_suppkey#494L, ps_supplycost#496]
Condition : (isnotnull(ps_suppkey#494L) AND isnotnull(ps_partkey#493L))

(23) Exchange
Input [3]: [ps_partkey#493L, ps_suppkey#494L, ps_supplycost#496]
Arguments: hashpartitioning(ps_suppkey#494L, 200), ENSURE_REQUIREMENTS, [plan_id=5318]

(24) Sort
Input [3]: [ps_partkey#493L, ps_suppkey#494L, ps_supplycost#496]
Arguments: [ps_suppkey#494L ASC NULLS FIRST], false, 0

(25) SortMergeJoin
Left keys [1]: [s_suppkey#498L]
Right keys [1]: [ps_suppkey#494L]
Join type: Inner
Join condition: None

(26) Project
Output [2]: [ps_partkey#493L, ps_supplycost#496]
Input [4]: [s_suppkey#498L, ps_partkey#493L, ps_suppkey#494L, ps_supplycost#496]

(27) HashAggregate
Input [2]: [ps_partkey#493L, ps_supplycost#496]
Keys [1]: [ps_partkey#493L]
Functions [1]: [partial_min(ps_supplycost#496)]
Aggregate Attributes [1]: [min#512]
Results [2]: [ps_partkey#493L, min#513]

(28) Exchange
Input [2]: [ps_partkey#493L, min#513]
Arguments: hashpartitioning(ps_partkey#493L, 200), ENSURE_REQUIREMENTS, [plan_id=5325]

(29) HashAggregate
Input [2]: [ps_partkey#493L, min#513]
Keys [1]: [ps_partkey#493L]
Functions [1]: [min(ps_supplycost#496)]
Aggregate Attributes [1]: [min(ps_supplycost#496)#491]
Results [2]: [min(ps_supplycost#496)#491 AS min(ps_supplycost)#492, ps_partkey#493L]

(30) Filter
Input [2]: [min(ps_supplycost)#492, ps_partkey#493L]
Condition : isnotnull(min(ps_supplycost)#492)

(31) Sort
Input [2]: [min(ps_supplycost)#492, ps_partkey#493L]
Arguments: [ps_partkey#493L ASC NULLS FIRST], false, 0

(32) SortMergeJoin
Left keys [1]: [p_partkey#199L]
Right keys [1]: [ps_partkey#493L]
Join type: Inner
Join condition: None

(33) Project
Output [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#492]
Input [4]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#492, ps_partkey#493L]

(34) Exchange
Input [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#492]
Arguments: hashpartitioning(p_partkey#199L, min(ps_supplycost)#492, 200), ENSURE_REQUIREMENTS, [plan_id=5338]

(35) Sort
Input [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#492]
Arguments: [p_partkey#199L ASC NULLS FIRST, min(ps_supplycost)#492 ASC NULLS FIRST], false, 0
...
(38) Exchange
Input [3]: [ps_partkey#215L, ps_suppkey#216L, ps_supplycost#218]
Arguments: hashpartitioning(ps_partkey#215L, ps_supplycost#218, 200), ENSURE_REQUIREMENTS, [plan_id=5337]
...
(40) SortMergeJoin
Left keys [2]: [p_partkey#199L, min(ps_supplycost)#492]
...
Input [6]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#492, ps_partkey#215L, ps_suppkey#216L, ps_supplycost#218]

(42) Exchange
Input [3]: [p_partkey#199L, p_mfgr#201, ps_suppkey#216L]
Arguments: hashpartitioning(ps_suppkey#216L, 200), ENSURE_REQUIREMENTS, [plan_id=5352]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=5343]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=5347]
...
Arguments: hashpartitioning(s_suppkey#208L, 200), ENSURE_REQUIREMENTS, [plan_id=5353]
