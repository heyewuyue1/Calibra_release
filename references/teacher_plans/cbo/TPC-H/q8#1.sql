...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=13231]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=13235]
...
Arguments: hashpartitioning(c_custkey#8L, 200), ENSURE_REQUIREMENTS, [plan_id=13240]
...
(18) Exchange
Input [3]: [o_orderkey#16L, o_custkey#17L, o_orderdate#20]
Arguments: hashpartitioning(o_custkey#17L, 200), ENSURE_REQUIREMENTS, [plan_id=13241]
...
Arguments: hashpartitioning(o_orderkey#16L, 200), ENSURE_REQUIREMENTS, [plan_id=13251]
...
PushedFilters: [IsNotNull(p_type), EqualTo(p_type,MEDIUM ANODIZED BRASS), IsNotNull(p_partkey)]
ReadSchema: struct<p_partkey:bigint,p_type:string>

(25) Filter
Input [2]: [p_partkey#199L, p_type#203]
Condition : ((isnotnull(p_type#203) AND (p_type#203 = MEDIUM ANODIZED BRASS)) AND isnotnull(p_partkey#199L))
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=13246]
...
Arguments: hashpartitioning(l_orderkey#25L, 200), ENSURE_REQUIREMENTS, [plan_id=13252]
...
Arguments: hashpartitioning(l_suppkey#27L, 200), ENSURE_REQUIREMENTS, [plan_id=13262]
...
Output [2]: [n_nationkey#1128L, n_name#1129]
Batched: true
Location: InMemoryFileIndex [file:/path/to/datasets/tpch_sf100_parquet/nation]
...
(41) Filter
Input [2]: [n_nationkey#1128L, n_name#1129]
Condition : isnotnull(n_nationkey#1128L)

(42) BroadcastExchange
Input [2]: [n_nationkey#1128L, n_name#1129]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, false]),false), [plan_id=13257]

(43) BroadcastHashJoin
Left keys [1]: [s_nationkey#211L]
Right keys [1]: [n_nationkey#1128L]
Join type: Inner
Join condition: None

(44) Project
Output [2]: [s_suppkey#208L, n_name#1129]
Input [4]: [s_suppkey#208L, s_nationkey#211L, n_nationkey#1128L, n_name#1129]

(45) Exchange
Input [2]: [s_suppkey#208L, n_name#1129]
Arguments: hashpartitioning(s_suppkey#208L, 200), ENSURE_REQUIREMENTS, [plan_id=13263]
...
(48) Project
Output [3]: [year(o_orderdate#20) AS o_year#1119, (l_extendedprice#30 * (1 - l_discount#31)) AS volume#1120, n_name#1129 AS nation#1121]
Input [6]: [o_orderdate#20, l_suppkey#27L, l_extendedprice#30, l_discount#31, s_suppkey#208L, n_name#1129]

(49) HashAggregate
Input [3]: [o_year#1119, volume#1120, nation#1121]
Keys [1]: [o_year#1119]
Functions [2]: [partial_sum(CASE WHEN (nation#1121 = ROMANIA) THEN volume#1120 ELSE 0 END), partial_sum(volume#1120)]
Aggregate Attributes [4]: [sum#1135, isEmpty#1136, sum#1137, isEmpty#1138]
Results [5]: [o_year#1119, sum#1139, isEmpty#1140, sum#1141, isEmpty#1142]

(50) Exchange
Input [5]: [o_year#1119, sum#1139, isEmpty#1140, sum#1141, isEmpty#1142]
Arguments: hashpartitioning(o_year#1119, 200), ENSURE_REQUIREMENTS, [plan_id=13270]

(51) HashAggregate
Input [5]: [o_year#1119, sum#1139, isEmpty#1140, sum#1141, isEmpty#1142]
Keys [1]: [o_year#1119]
Functions [2]: [sum(CASE WHEN (nation#1121 = ROMANIA) THEN volume#1120 ELSE 0 END), sum(volume#1120)]
Aggregate Attributes [2]: [sum(CASE WHEN (nation#1121 = ROMANIA) THEN volume#1120 ELSE 0 END)#1134, sum(volume#1120)#1133]
Results [2]: [o_year#1119, (sum(CASE WHEN (nation#1121 = ROMANIA) THEN volume#1120 ELSE 0 END)#1134 / sum(volume#1120)#1133) AS mkt_share#1122]

(52) Exchange
Input [2]: [o_year#1119, mkt_share#1122]
Arguments: rangepartitioning(o_year#1119 ASC NULLS FIRST, 200), ENSURE_REQUIREMENTS, [plan_id=13273]

(53) Sort
Input [2]: [o_year#1119, mkt_share#1122]
Arguments: [o_year#1119 ASC NULLS FIRST], true, 0

(54) AdaptiveSparkPlan
Output [2]: [o_year#1119, mkt_share#1122]
Arguments: isFinalPlan=false
Execution Time: 25.149
