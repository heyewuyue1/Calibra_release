...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=13581]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=13585]
...
Arguments: hashpartitioning(c_custkey#8L, 200), ENSURE_REQUIREMENTS, [plan_id=13590]
...
(18) Exchange
Input [3]: [o_orderkey#16L, o_custkey#17L, o_orderdate#20]
Arguments: hashpartitioning(o_custkey#17L, 200), ENSURE_REQUIREMENTS, [plan_id=13591]
...
Arguments: hashpartitioning(o_orderkey#16L, 200), ENSURE_REQUIREMENTS, [plan_id=13601]
...
PushedFilters: [IsNotNull(p_type), EqualTo(p_type,PROMO BURNISHED TIN), IsNotNull(p_partkey)]
ReadSchema: struct<p_partkey:bigint,p_type:string>

(25) Filter
Input [2]: [p_partkey#199L, p_type#203]
Condition : ((isnotnull(p_type#203) AND (p_type#203 = PROMO BURNISHED TIN)) AND isnotnull(p_partkey#199L))
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=13596]
...
Arguments: hashpartitioning(l_orderkey#25L, 200), ENSURE_REQUIREMENTS, [plan_id=13602]
...
Arguments: hashpartitioning(l_suppkey#27L, 200), ENSURE_REQUIREMENTS, [plan_id=13612]
...
Output [2]: [n_nationkey#1156L, n_name#1157]
Batched: true
Location: InMemoryFileIndex [file:/path/to/datasets/tpch_sf100_parquet/nation]
...
(41) Filter
Input [2]: [n_nationkey#1156L, n_name#1157]
Condition : isnotnull(n_nationkey#1156L)

(42) BroadcastExchange
Input [2]: [n_nationkey#1156L, n_name#1157]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, false]),false), [plan_id=13607]

(43) BroadcastHashJoin
Left keys [1]: [s_nationkey#211L]
Right keys [1]: [n_nationkey#1156L]
Join type: Inner
Join condition: None

(44) Project
Output [2]: [s_suppkey#208L, n_name#1157]
Input [4]: [s_suppkey#208L, s_nationkey#211L, n_nationkey#1156L, n_name#1157]

(45) Exchange
Input [2]: [s_suppkey#208L, n_name#1157]
Arguments: hashpartitioning(s_suppkey#208L, 200), ENSURE_REQUIREMENTS, [plan_id=13613]
...
(48) Project
Output [3]: [year(o_orderdate#20) AS o_year#1147, (l_extendedprice#30 * (1 - l_discount#31)) AS volume#1148, n_name#1157 AS nation#1149]
Input [6]: [o_orderdate#20, l_suppkey#27L, l_extendedprice#30, l_discount#31, s_suppkey#208L, n_name#1157]

(49) HashAggregate
Input [3]: [o_year#1147, volume#1148, nation#1149]
Keys [1]: [o_year#1147]
Functions [2]: [partial_sum(CASE WHEN (nation#1149 = INDIA) THEN volume#1148 ELSE 0 END), partial_sum(volume#1148)]
Aggregate Attributes [4]: [sum#1163, isEmpty#1164, sum#1165, isEmpty#1166]
Results [5]: [o_year#1147, sum#1167, isEmpty#1168, sum#1169, isEmpty#1170]

(50) Exchange
Input [5]: [o_year#1147, sum#1167, isEmpty#1168, sum#1169, isEmpty#1170]
Arguments: hashpartitioning(o_year#1147, 200), ENSURE_REQUIREMENTS, [plan_id=13620]

(51) HashAggregate
Input [5]: [o_year#1147, sum#1167, isEmpty#1168, sum#1169, isEmpty#1170]
Keys [1]: [o_year#1147]
Functions [2]: [sum(CASE WHEN (nation#1149 = INDIA) THEN volume#1148 ELSE 0 END), sum(volume#1148)]
Aggregate Attributes [2]: [sum(CASE WHEN (nation#1149 = INDIA) THEN volume#1148 ELSE 0 END)#1162, sum(volume#1148)#1161]
Results [2]: [o_year#1147, (sum(CASE WHEN (nation#1149 = INDIA) THEN volume#1148 ELSE 0 END)#1162 / sum(volume#1148)#1161) AS mkt_share#1150]

(52) Exchange
Input [2]: [o_year#1147, mkt_share#1150]
Arguments: rangepartitioning(o_year#1147 ASC NULLS FIRST, 200), ENSURE_REQUIREMENTS, [plan_id=13623]

(53) Sort
Input [2]: [o_year#1147, mkt_share#1150]
Arguments: [o_year#1147 ASC NULLS FIRST], true, 0

(54) AdaptiveSparkPlan
Output [2]: [o_year#1147, mkt_share#1150]
Arguments: isFinalPlan=false
Execution Time: 25.115
