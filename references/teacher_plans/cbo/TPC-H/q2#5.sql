...
PushedFilters: [IsNotNull(p_size), IsNotNull(p_type), EqualTo(p_size,39), StringEndsWith(p_type,STEEL), IsNotNull(p_partkey)]
...
Condition : ((((isnotnull(p_size#204) AND isnotnull(p_type#203)) AND (p_size#204 = 39)) AND EndsWith(p_type#203, STEEL)) AND isnotnull(p_partkey#199L))
...
Arguments: hashpartitioning(p_partkey#199L, 200), ENSURE_REQUIREMENTS, [plan_id=4172]
...
Output [2]: [n_nationkey#406L, n_regionkey#408L]
Batched: true
...
(7) Filter
Input [2]: [n_nationkey#406L, n_regionkey#408L]
Condition : (isnotnull(n_nationkey#406L) AND isnotnull(n_regionkey#408L))

(8) Scan parquet spark_catalog.tpch_sf100.region
Output [2]: [r_regionkey#410L, r_name#411]
Batched: true
...
(9) Filter
Input [2]: [r_regionkey#410L, r_name#411]
Condition : ((isnotnull(r_name#411) AND (r_name#411 = EUROPE)) AND isnotnull(r_regionkey#410L))

(10) Project
Output [1]: [r_regionkey#410L]
Input [2]: [r_regionkey#410L, r_name#411]

(11) BroadcastExchange
Input [1]: [r_regionkey#410L]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=4150]

(12) BroadcastHashJoin
Left keys [1]: [n_regionkey#408L]
Right keys [1]: [r_regionkey#410L]
Join type: Inner
Join condition: None

(13) Project
Output [1]: [n_nationkey#406L]
Input [3]: [n_nationkey#406L, n_regionkey#408L, r_regionkey#410L]

(14) BroadcastExchange
Input [1]: [n_nationkey#406L]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=4154]

(15) Scan parquet spark_catalog.tpch_sf100.supplier
Output [2]: [s_suppkey#399L, s_nationkey#402L]
Batched: true
...
(16) Filter
Input [2]: [s_suppkey#399L, s_nationkey#402L]
Condition : (isnotnull(s_suppkey#399L) AND isnotnull(s_nationkey#402L))

(17) BroadcastHashJoin
Left keys [1]: [n_nationkey#406L]
Right keys [1]: [s_nationkey#402L]
Join type: Inner
Join condition: None

(18) Project
Output [1]: [s_suppkey#399L]
Input [3]: [n_nationkey#406L, s_suppkey#399L, s_nationkey#402L]

(19) Exchange
Input [1]: [s_suppkey#399L]
Arguments: hashpartitioning(s_suppkey#399L, 200), ENSURE_REQUIREMENTS, [plan_id=4159]

(20) Sort
Input [1]: [s_suppkey#399L]
Arguments: [s_suppkey#399L ASC NULLS FIRST], false, 0

(21) Scan parquet spark_catalog.tpch_sf100.partsupp
Output [3]: [ps_partkey#394L, ps_suppkey#395L, ps_supplycost#397]
Batched: true
...
(22) Filter
Input [3]: [ps_partkey#394L, ps_suppkey#395L, ps_supplycost#397]
Condition : (isnotnull(ps_suppkey#395L) AND isnotnull(ps_partkey#394L))

(23) Exchange
Input [3]: [ps_partkey#394L, ps_suppkey#395L, ps_supplycost#397]
Arguments: hashpartitioning(ps_suppkey#395L, 200), ENSURE_REQUIREMENTS, [plan_id=4160]

(24) Sort
Input [3]: [ps_partkey#394L, ps_suppkey#395L, ps_supplycost#397]
Arguments: [ps_suppkey#395L ASC NULLS FIRST], false, 0

(25) SortMergeJoin
Left keys [1]: [s_suppkey#399L]
Right keys [1]: [ps_suppkey#395L]
Join type: Inner
Join condition: None

(26) Project
Output [2]: [ps_partkey#394L, ps_supplycost#397]
Input [4]: [s_suppkey#399L, ps_partkey#394L, ps_suppkey#395L, ps_supplycost#397]

(27) HashAggregate
Input [2]: [ps_partkey#394L, ps_supplycost#397]
Keys [1]: [ps_partkey#394L]
Functions [1]: [partial_min(ps_supplycost#397)]
Aggregate Attributes [1]: [min#413]
Results [2]: [ps_partkey#394L, min#414]

(28) Exchange
Input [2]: [ps_partkey#394L, min#414]
Arguments: hashpartitioning(ps_partkey#394L, 200), ENSURE_REQUIREMENTS, [plan_id=4167]

(29) HashAggregate
Input [2]: [ps_partkey#394L, min#414]
Keys [1]: [ps_partkey#394L]
Functions [1]: [min(ps_supplycost#397)]
Aggregate Attributes [1]: [min(ps_supplycost#397)#392]
Results [2]: [min(ps_supplycost#397)#392 AS min(ps_supplycost)#393, ps_partkey#394L]

(30) Filter
Input [2]: [min(ps_supplycost)#393, ps_partkey#394L]
Condition : isnotnull(min(ps_supplycost)#393)

(31) Sort
Input [2]: [min(ps_supplycost)#393, ps_partkey#394L]
Arguments: [ps_partkey#394L ASC NULLS FIRST], false, 0

(32) SortMergeJoin
Left keys [1]: [p_partkey#199L]
Right keys [1]: [ps_partkey#394L]
Join type: Inner
Join condition: None

(33) Project
Output [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#393]
Input [4]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#393, ps_partkey#394L]

(34) Exchange
Input [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#393]
Arguments: hashpartitioning(p_partkey#199L, min(ps_supplycost)#393, 200), ENSURE_REQUIREMENTS, [plan_id=4180]

(35) Sort
Input [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#393]
Arguments: [p_partkey#199L ASC NULLS FIRST, min(ps_supplycost)#393 ASC NULLS FIRST], false, 0
...
(38) Exchange
Input [3]: [ps_partkey#215L, ps_suppkey#216L, ps_supplycost#218]
Arguments: hashpartitioning(ps_partkey#215L, ps_supplycost#218, 200), ENSURE_REQUIREMENTS, [plan_id=4179]
...
(40) SortMergeJoin
Left keys [2]: [p_partkey#199L, min(ps_supplycost)#393]
...
Input [6]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#393, ps_partkey#215L, ps_suppkey#216L, ps_supplycost#218]

(42) Exchange
Input [3]: [p_partkey#199L, p_mfgr#201, ps_suppkey#216L]
Arguments: hashpartitioning(ps_suppkey#216L, 200), ENSURE_REQUIREMENTS, [plan_id=4194]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=4185]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=4189]
...
Arguments: hashpartitioning(s_suppkey#208L, 200), ENSURE_REQUIREMENTS, [plan_id=4195]
