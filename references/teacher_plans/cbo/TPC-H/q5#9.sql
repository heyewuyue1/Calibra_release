...
(3) Exchange
Input [2]: [c_custkey#8L, c_nationkey#11L]
Arguments: hashpartitioning(c_custkey#8L, 200), ENSURE_REQUIREMENTS, [plan_id=9879]
...
PushedFilters: [IsNotNull(o_orderdate), GreaterThanOrEqual(o_orderdate,1995-01-01), LessThan(o_orderdate,1996-01-01), IsNotNull(o_custkey), IsNotNull(o_orderkey)]
ReadSchema: struct<o_orderkey:bigint,o_custkey:bigint,o_orderdate:date>

(6) Filter
Input [3]: [o_orderkey#16L, o_custkey#17L, o_orderdate#20]
Condition : ((((isnotnull(o_orderdate#20) AND (o_orderdate#20 >= 1995-01-01)) AND (o_orderdate#20 < 1996-01-01)) AND isnotnull(o_custkey#17L)) AND isnotnull(o_orderkey#16L))
...
Arguments: hashpartitioning(o_custkey#17L, 200), ENSURE_REQUIREMENTS, [plan_id=9880]
...
Arguments: hashpartitioning(o_orderkey#16L, 200), ENSURE_REQUIREMENTS, [plan_id=9887]
...
(16) Exchange
Input [4]: [l_orderkey#25L, l_suppkey#27L, l_extendedprice#30, l_discount#31]
Arguments: hashpartitioning(l_orderkey#25L, 200), ENSURE_REQUIREMENTS, [plan_id=9888]
...
Arguments: hashpartitioning(l_suppkey#27L, c_nationkey#11L, 200), ENSURE_REQUIREMENTS, [plan_id=9902]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=9893]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=9897]
...
Arguments: hashpartitioning(s_suppkey#208L, s_nationkey#211L, 200), ENSURE_REQUIREMENTS, [plan_id=9903]
...
Aggregate Attributes [2]: [sum#843, isEmpty#844]
Results [3]: [n_name#42, sum#845, isEmpty#846]

(40) Exchange
Input [3]: [n_name#42, sum#845, isEmpty#846]
Arguments: hashpartitioning(n_name#42, 200), ENSURE_REQUIREMENTS, [plan_id=9910]

(41) HashAggregate
Input [3]: [n_name#42, sum#845, isEmpty#846]
Keys [1]: [n_name#42]
Functions [1]: [sum((l_extendedprice#30 * (1 - l_discount#31)))]
Aggregate Attributes [1]: [sum((l_extendedprice#30 * (1 - l_discount#31)))#842]
Results [2]: [n_name#42, sum((l_extendedprice#30 * (1 - l_discount#31)))#842 AS revenue#836]

(42) Exchange
Input [2]: [n_name#42, revenue#836]
Arguments: rangepartitioning(revenue#836 DESC NULLS LAST, 200), ENSURE_REQUIREMENTS, [plan_id=9913]

(43) Sort
Input [2]: [n_name#42, revenue#836]
Arguments: [revenue#836 DESC NULLS LAST], true, 0

(44) AdaptiveSparkPlan
Output [2]: [n_name#42, revenue#836]
Arguments: isFinalPlan=false
Execution Time: 43.072
