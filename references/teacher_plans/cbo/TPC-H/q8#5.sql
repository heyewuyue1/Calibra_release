...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=14631]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=14635]
...
Arguments: hashpartitioning(c_custkey#8L, 200), ENSURE_REQUIREMENTS, [plan_id=14640]
...
(18) Exchange
Input [3]: [o_orderkey#16L, o_custkey#17L, o_orderdate#20]
Arguments: hashpartitioning(o_custkey#17L, 200), ENSURE_REQUIREMENTS, [plan_id=14641]
...
Arguments: hashpartitioning(o_orderkey#16L, 200), ENSURE_REQUIREMENTS, [plan_id=14651]
...
PushedFilters: [IsNotNull(p_type), EqualTo(p_type,ECONOMY ANODIZED NICKEL), IsNotNull(p_partkey)]
ReadSchema: struct<p_partkey:bigint,p_type:string>

(25) Filter
Input [2]: [p_partkey#199L, p_type#203]
Condition : ((isnotnull(p_type#203) AND (p_type#203 = ECONOMY ANODIZED NICKEL)) AND isnotnull(p_partkey#199L))
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=14646]
...
Arguments: hashpartitioning(l_orderkey#25L, 200), ENSURE_REQUIREMENTS, [plan_id=14652]
...
Arguments: hashpartitioning(l_suppkey#27L, 200), ENSURE_REQUIREMENTS, [plan_id=14662]
...
Output [2]: [n_nationkey#1240L, n_name#1241]
Batched: true
Location: InMemoryFileIndex [file:/path/to/datasets/tpch_sf100_parquet/nation]
...
(41) Filter
Input [2]: [n_nationkey#1240L, n_name#1241]
Condition : isnotnull(n_nationkey#1240L)

(42) BroadcastExchange
Input [2]: [n_nationkey#1240L, n_name#1241]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, false]),false), [plan_id=14657]

(43) BroadcastHashJoin
Left keys [1]: [s_nationkey#211L]
Right keys [1]: [n_nationkey#1240L]
Join type: Inner
Join condition: None

(44) Project
Output [2]: [s_suppkey#208L, n_name#1241]
Input [4]: [s_suppkey#208L, s_nationkey#211L, n_nationkey#1240L, n_name#1241]

(45) Exchange
Input [2]: [s_suppkey#208L, n_name#1241]
Arguments: hashpartitioning(s_suppkey#208L, 200), ENSURE_REQUIREMENTS, [plan_id=14663]
...
(48) Project
Output [3]: [year(o_orderdate#20) AS o_year#1231, (l_extendedprice#30 * (1 - l_discount#31)) AS volume#1232, n_name#1241 AS nation#1233]
Input [6]: [o_orderdate#20, l_suppkey#27L, l_extendedprice#30, l_discount#31, s_suppkey#208L, n_name#1241]

(49) HashAggregate
Input [3]: [o_year#1231, volume#1232, nation#1233]
Keys [1]: [o_year#1231]
Functions [2]: [partial_sum(CASE WHEN (nation#1233 = INDONESIA) THEN volume#1232 ELSE 0 END), partial_sum(volume#1232)]
Aggregate Attributes [4]: [sum#1247, isEmpty#1248, sum#1249, isEmpty#1250]
Results [5]: [o_year#1231, sum#1251, isEmpty#1252, sum#1253, isEmpty#1254]

(50) Exchange
Input [5]: [o_year#1231, sum#1251, isEmpty#1252, sum#1253, isEmpty#1254]
Arguments: hashpartitioning(o_year#1231, 200), ENSURE_REQUIREMENTS, [plan_id=14670]

(51) HashAggregate
Input [5]: [o_year#1231, sum#1251, isEmpty#1252, sum#1253, isEmpty#1254]
Keys [1]: [o_year#1231]
Functions [2]: [sum(CASE WHEN (nation#1233 = INDONESIA) THEN volume#1232 ELSE 0 END), sum(volume#1232)]
Aggregate Attributes [2]: [sum(CASE WHEN (nation#1233 = INDONESIA) THEN volume#1232 ELSE 0 END)#1246, sum(volume#1232)#1245]
Results [2]: [o_year#1231, (sum(CASE WHEN (nation#1233 = INDONESIA) THEN volume#1232 ELSE 0 END)#1246 / sum(volume#1232)#1245) AS mkt_share#1234]

(52) Exchange
Input [2]: [o_year#1231, mkt_share#1234]
Arguments: rangepartitioning(o_year#1231 ASC NULLS FIRST, 200), ENSURE_REQUIREMENTS, [plan_id=14673]

(53) Sort
Input [2]: [o_year#1231, mkt_share#1234]
Arguments: [o_year#1231 ASC NULLS FIRST], true, 0

(54) AdaptiveSparkPlan
Output [2]: [o_year#1231, mkt_share#1234]
Arguments: isFinalPlan=false
Execution Time: 25.669
