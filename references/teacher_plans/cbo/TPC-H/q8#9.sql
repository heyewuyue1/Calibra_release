...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=16031]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=16035]
...
Arguments: hashpartitioning(c_custkey#8L, 200), ENSURE_REQUIREMENTS, [plan_id=16040]
...
(18) Exchange
Input [3]: [o_orderkey#16L, o_custkey#17L, o_orderdate#20]
Arguments: hashpartitioning(o_custkey#17L, 200), ENSURE_REQUIREMENTS, [plan_id=16041]
...
Arguments: hashpartitioning(o_orderkey#16L, 200), ENSURE_REQUIREMENTS, [plan_id=16051]
...
PushedFilters: [IsNotNull(p_type), EqualTo(p_type,MEDIUM PLATED NICKEL), IsNotNull(p_partkey)]
ReadSchema: struct<p_partkey:bigint,p_type:string>

(25) Filter
Input [2]: [p_partkey#199L, p_type#203]
Condition : ((isnotnull(p_type#203) AND (p_type#203 = MEDIUM PLATED NICKEL)) AND isnotnull(p_partkey#199L))
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=16046]
...
Arguments: hashpartitioning(l_orderkey#25L, 200), ENSURE_REQUIREMENTS, [plan_id=16052]
...
Arguments: hashpartitioning(l_suppkey#27L, 200), ENSURE_REQUIREMENTS, [plan_id=16062]
...
Output [2]: [n_nationkey#1352L, n_name#1353]
Batched: true
Location: InMemoryFileIndex [file:/path/to/datasets/tpch_sf100_parquet/nation]
...
(41) Filter
Input [2]: [n_nationkey#1352L, n_name#1353]
Condition : isnotnull(n_nationkey#1352L)

(42) BroadcastExchange
Input [2]: [n_nationkey#1352L, n_name#1353]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, false]),false), [plan_id=16057]

(43) BroadcastHashJoin
Left keys [1]: [s_nationkey#211L]
Right keys [1]: [n_nationkey#1352L]
Join type: Inner
Join condition: None

(44) Project
Output [2]: [s_suppkey#208L, n_name#1353]
Input [4]: [s_suppkey#208L, s_nationkey#211L, n_nationkey#1352L, n_name#1353]

(45) Exchange
Input [2]: [s_suppkey#208L, n_name#1353]
Arguments: hashpartitioning(s_suppkey#208L, 200), ENSURE_REQUIREMENTS, [plan_id=16063]
...
(48) Project
Output [3]: [year(o_orderdate#20) AS o_year#1343, (l_extendedprice#30 * (1 - l_discount#31)) AS volume#1344, n_name#1353 AS nation#1345]
Input [6]: [o_orderdate#20, l_suppkey#27L, l_extendedprice#30, l_discount#31, s_suppkey#208L, n_name#1353]

(49) HashAggregate
Input [3]: [o_year#1343, volume#1344, nation#1345]
Keys [1]: [o_year#1343]
Functions [2]: [partial_sum(CASE WHEN (nation#1345 = BRAZIL) THEN volume#1344 ELSE 0 END), partial_sum(volume#1344)]
Aggregate Attributes [4]: [sum#1359, isEmpty#1360, sum#1361, isEmpty#1362]
Results [5]: [o_year#1343, sum#1363, isEmpty#1364, sum#1365, isEmpty#1366]

(50) Exchange
Input [5]: [o_year#1343, sum#1363, isEmpty#1364, sum#1365, isEmpty#1366]
Arguments: hashpartitioning(o_year#1343, 200), ENSURE_REQUIREMENTS, [plan_id=16070]

(51) HashAggregate
Input [5]: [o_year#1343, sum#1363, isEmpty#1364, sum#1365, isEmpty#1366]
Keys [1]: [o_year#1343]
Functions [2]: [sum(CASE WHEN (nation#1345 = BRAZIL) THEN volume#1344 ELSE 0 END), sum(volume#1344)]
Aggregate Attributes [2]: [sum(CASE WHEN (nation#1345 = BRAZIL) THEN volume#1344 ELSE 0 END)#1358, sum(volume#1344)#1357]
Results [2]: [o_year#1343, (sum(CASE WHEN (nation#1345 = BRAZIL) THEN volume#1344 ELSE 0 END)#1358 / sum(volume#1344)#1357) AS mkt_share#1346]

(52) Exchange
Input [2]: [o_year#1343, mkt_share#1346]
Arguments: rangepartitioning(o_year#1343 ASC NULLS FIRST, 200), ENSURE_REQUIREMENTS, [plan_id=16073]

(53) Sort
Input [2]: [o_year#1343, mkt_share#1346]
Arguments: [o_year#1343 ASC NULLS FIRST], true, 0

(54) AdaptiveSparkPlan
Output [2]: [o_year#1343, mkt_share#1346]
Arguments: isFinalPlan=false
Execution Time: 25.998
