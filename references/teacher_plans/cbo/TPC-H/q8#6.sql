...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=14981]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=14985]
...
Arguments: hashpartitioning(c_custkey#8L, 200), ENSURE_REQUIREMENTS, [plan_id=14990]
...
(18) Exchange
Input [3]: [o_orderkey#16L, o_custkey#17L, o_orderdate#20]
Arguments: hashpartitioning(o_custkey#17L, 200), ENSURE_REQUIREMENTS, [plan_id=14991]
...
Arguments: hashpartitioning(o_orderkey#16L, 200), ENSURE_REQUIREMENTS, [plan_id=15001]
...
PushedFilters: [IsNotNull(p_type), EqualTo(p_type,LARGE POLISHED NICKEL), IsNotNull(p_partkey)]
ReadSchema: struct<p_partkey:bigint,p_type:string>

(25) Filter
Input [2]: [p_partkey#199L, p_type#203]
Condition : ((isnotnull(p_type#203) AND (p_type#203 = LARGE POLISHED NICKEL)) AND isnotnull(p_partkey#199L))
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=14996]
...
Arguments: hashpartitioning(l_orderkey#25L, 200), ENSURE_REQUIREMENTS, [plan_id=15002]
...
Arguments: hashpartitioning(l_suppkey#27L, 200), ENSURE_REQUIREMENTS, [plan_id=15012]
...
Output [2]: [n_nationkey#1268L, n_name#1269]
Batched: true
Location: InMemoryFileIndex [file:/path/to/datasets/tpch_sf100_parquet/nation]
...
(41) Filter
Input [2]: [n_nationkey#1268L, n_name#1269]
Condition : isnotnull(n_nationkey#1268L)

(42) BroadcastExchange
Input [2]: [n_nationkey#1268L, n_name#1269]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, false]),false), [plan_id=15007]

(43) BroadcastHashJoin
Left keys [1]: [s_nationkey#211L]
Right keys [1]: [n_nationkey#1268L]
Join type: Inner
Join condition: None

(44) Project
Output [2]: [s_suppkey#208L, n_name#1269]
Input [4]: [s_suppkey#208L, s_nationkey#211L, n_nationkey#1268L, n_name#1269]

(45) Exchange
Input [2]: [s_suppkey#208L, n_name#1269]
Arguments: hashpartitioning(s_suppkey#208L, 200), ENSURE_REQUIREMENTS, [plan_id=15013]
...
(48) Project
Output [3]: [year(o_orderdate#20) AS o_year#1259, (l_extendedprice#30 * (1 - l_discount#31)) AS volume#1260, n_name#1269 AS nation#1261]
Input [6]: [o_orderdate#20, l_suppkey#27L, l_extendedprice#30, l_discount#31, s_suppkey#208L, n_name#1269]

(49) HashAggregate
Input [3]: [o_year#1259, volume#1260, nation#1261]
Keys [1]: [o_year#1259]
Functions [2]: [partial_sum(CASE WHEN (nation#1261 = ARGENTINA) THEN volume#1260 ELSE 0 END), partial_sum(volume#1260)]
Aggregate Attributes [4]: [sum#1275, isEmpty#1276, sum#1277, isEmpty#1278]
Results [5]: [o_year#1259, sum#1279, isEmpty#1280, sum#1281, isEmpty#1282]

(50) Exchange
Input [5]: [o_year#1259, sum#1279, isEmpty#1280, sum#1281, isEmpty#1282]
Arguments: hashpartitioning(o_year#1259, 200), ENSURE_REQUIREMENTS, [plan_id=15020]

(51) HashAggregate
Input [5]: [o_year#1259, sum#1279, isEmpty#1280, sum#1281, isEmpty#1282]
Keys [1]: [o_year#1259]
Functions [2]: [sum(CASE WHEN (nation#1261 = ARGENTINA) THEN volume#1260 ELSE 0 END), sum(volume#1260)]
Aggregate Attributes [2]: [sum(CASE WHEN (nation#1261 = ARGENTINA) THEN volume#1260 ELSE 0 END)#1274, sum(volume#1260)#1273]
Results [2]: [o_year#1259, (sum(CASE WHEN (nation#1261 = ARGENTINA) THEN volume#1260 ELSE 0 END)#1274 / sum(volume#1260)#1273) AS mkt_share#1262]

(52) Exchange
Input [2]: [o_year#1259, mkt_share#1262]
Arguments: rangepartitioning(o_year#1259 ASC NULLS FIRST, 200), ENSURE_REQUIREMENTS, [plan_id=15023]

(53) Sort
Input [2]: [o_year#1259, mkt_share#1262]
Arguments: [o_year#1259 ASC NULLS FIRST], true, 0

(54) AdaptiveSparkPlan
Output [2]: [o_year#1259, mkt_share#1262]
Arguments: isFinalPlan=false
Execution Time: 25.161
