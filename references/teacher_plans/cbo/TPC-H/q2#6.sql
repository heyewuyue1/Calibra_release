...
PushedFilters: [IsNotNull(p_size), IsNotNull(p_type), EqualTo(p_size,27), StringEndsWith(p_type,BRASS), IsNotNull(p_partkey)]
...
Condition : ((((isnotnull(p_size#204) AND isnotnull(p_type#203)) AND (p_size#204 = 27)) AND EndsWith(p_type#203, BRASS)) AND isnotnull(p_partkey#199L))
...
Arguments: hashpartitioning(p_partkey#199L, 200), ENSURE_REQUIREMENTS, [plan_id=4558]
...
Output [2]: [n_nationkey#439L, n_regionkey#441L]
Batched: true
...
(7) Filter
Input [2]: [n_nationkey#439L, n_regionkey#441L]
Condition : (isnotnull(n_nationkey#439L) AND isnotnull(n_regionkey#441L))

(8) Scan parquet spark_catalog.tpch_sf100.region
Output [2]: [r_regionkey#443L, r_name#444]
Batched: true
...
(9) Filter
Input [2]: [r_regionkey#443L, r_name#444]
Condition : ((isnotnull(r_name#444) AND (r_name#444 = AFRICA)) AND isnotnull(r_regionkey#443L))

(10) Project
Output [1]: [r_regionkey#443L]
Input [2]: [r_regionkey#443L, r_name#444]

(11) BroadcastExchange
Input [1]: [r_regionkey#443L]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=4536]

(12) BroadcastHashJoin
Left keys [1]: [n_regionkey#441L]
Right keys [1]: [r_regionkey#443L]
Join type: Inner
Join condition: None

(13) Project
Output [1]: [n_nationkey#439L]
Input [3]: [n_nationkey#439L, n_regionkey#441L, r_regionkey#443L]

(14) BroadcastExchange
Input [1]: [n_nationkey#439L]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=4540]

(15) Scan parquet spark_catalog.tpch_sf100.supplier
Output [2]: [s_suppkey#432L, s_nationkey#435L]
Batched: true
...
(16) Filter
Input [2]: [s_suppkey#432L, s_nationkey#435L]
Condition : (isnotnull(s_suppkey#432L) AND isnotnull(s_nationkey#435L))

(17) BroadcastHashJoin
Left keys [1]: [n_nationkey#439L]
Right keys [1]: [s_nationkey#435L]
Join type: Inner
Join condition: None

(18) Project
Output [1]: [s_suppkey#432L]
Input [3]: [n_nationkey#439L, s_suppkey#432L, s_nationkey#435L]

(19) Exchange
Input [1]: [s_suppkey#432L]
Arguments: hashpartitioning(s_suppkey#432L, 200), ENSURE_REQUIREMENTS, [plan_id=4545]

(20) Sort
Input [1]: [s_suppkey#432L]
Arguments: [s_suppkey#432L ASC NULLS FIRST], false, 0

(21) Scan parquet spark_catalog.tpch_sf100.partsupp
Output [3]: [ps_partkey#427L, ps_suppkey#428L, ps_supplycost#430]
Batched: true
...
(22) Filter
Input [3]: [ps_partkey#427L, ps_suppkey#428L, ps_supplycost#430]
Condition : (isnotnull(ps_suppkey#428L) AND isnotnull(ps_partkey#427L))

(23) Exchange
Input [3]: [ps_partkey#427L, ps_suppkey#428L, ps_supplycost#430]
Arguments: hashpartitioning(ps_suppkey#428L, 200), ENSURE_REQUIREMENTS, [plan_id=4546]

(24) Sort
Input [3]: [ps_partkey#427L, ps_suppkey#428L, ps_supplycost#430]
Arguments: [ps_suppkey#428L ASC NULLS FIRST], false, 0

(25) SortMergeJoin
Left keys [1]: [s_suppkey#432L]
Right keys [1]: [ps_suppkey#428L]
Join type: Inner
Join condition: None

(26) Project
Output [2]: [ps_partkey#427L, ps_supplycost#430]
Input [4]: [s_suppkey#432L, ps_partkey#427L, ps_suppkey#428L, ps_supplycost#430]

(27) HashAggregate
Input [2]: [ps_partkey#427L, ps_supplycost#430]
Keys [1]: [ps_partkey#427L]
Functions [1]: [partial_min(ps_supplycost#430)]
Aggregate Attributes [1]: [min#446]
Results [2]: [ps_partkey#427L, min#447]

(28) Exchange
Input [2]: [ps_partkey#427L, min#447]
Arguments: hashpartitioning(ps_partkey#427L, 200), ENSURE_REQUIREMENTS, [plan_id=4553]

(29) HashAggregate
Input [2]: [ps_partkey#427L, min#447]
Keys [1]: [ps_partkey#427L]
Functions [1]: [min(ps_supplycost#430)]
Aggregate Attributes [1]: [min(ps_supplycost#430)#425]
Results [2]: [min(ps_supplycost#430)#425 AS min(ps_supplycost)#426, ps_partkey#427L]

(30) Filter
Input [2]: [min(ps_supplycost)#426, ps_partkey#427L]
Condition : isnotnull(min(ps_supplycost)#426)

(31) Sort
Input [2]: [min(ps_supplycost)#426, ps_partkey#427L]
Arguments: [ps_partkey#427L ASC NULLS FIRST], false, 0

(32) SortMergeJoin
Left keys [1]: [p_partkey#199L]
Right keys [1]: [ps_partkey#427L]
Join type: Inner
Join condition: None

(33) Project
Output [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#426]
Input [4]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#426, ps_partkey#427L]

(34) Exchange
Input [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#426]
Arguments: hashpartitioning(p_partkey#199L, min(ps_supplycost)#426, 200), ENSURE_REQUIREMENTS, [plan_id=4566]

(35) Sort
Input [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#426]
Arguments: [p_partkey#199L ASC NULLS FIRST, min(ps_supplycost)#426 ASC NULLS FIRST], false, 0
...
(38) Exchange
Input [3]: [ps_partkey#215L, ps_suppkey#216L, ps_supplycost#218]
Arguments: hashpartitioning(ps_partkey#215L, ps_supplycost#218, 200), ENSURE_REQUIREMENTS, [plan_id=4565]
...
(40) SortMergeJoin
Left keys [2]: [p_partkey#199L, min(ps_supplycost)#426]
...
Input [6]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#426, ps_partkey#215L, ps_suppkey#216L, ps_supplycost#218]

(42) Exchange
Input [3]: [p_partkey#199L, p_mfgr#201, ps_suppkey#216L]
Arguments: hashpartitioning(ps_suppkey#216L, 200), ENSURE_REQUIREMENTS, [plan_id=4580]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=4571]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=4575]
...
Arguments: hashpartitioning(s_suppkey#208L, 200), ENSURE_REQUIREMENTS, [plan_id=4581]
