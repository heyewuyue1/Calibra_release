...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=15331]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=15335]
...
Arguments: hashpartitioning(c_custkey#8L, 200), ENSURE_REQUIREMENTS, [plan_id=15340]
...
(18) Exchange
Input [3]: [o_orderkey#16L, o_custkey#17L, o_orderdate#20]
Arguments: hashpartitioning(o_custkey#17L, 200), ENSURE_REQUIREMENTS, [plan_id=15341]
...
Arguments: hashpartitioning(o_orderkey#16L, 200), ENSURE_REQUIREMENTS, [plan_id=15351]
...
PushedFilters: [IsNotNull(p_type), EqualTo(p_type,LARGE BURNISHED NICKEL), IsNotNull(p_partkey)]
ReadSchema: struct<p_partkey:bigint,p_type:string>

(25) Filter
Input [2]: [p_partkey#199L, p_type#203]
Condition : ((isnotnull(p_type#203) AND (p_type#203 = LARGE BURNISHED NICKEL)) AND isnotnull(p_partkey#199L))
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=15346]
...
Arguments: hashpartitioning(l_orderkey#25L, 200), ENSURE_REQUIREMENTS, [plan_id=15352]
...
Arguments: hashpartitioning(l_suppkey#27L, 200), ENSURE_REQUIREMENTS, [plan_id=15362]
...
Output [2]: [n_nationkey#1296L, n_name#1297]
Batched: true
Location: InMemoryFileIndex [file:/path/to/datasets/tpch_sf100_parquet/nation]
...
(41) Filter
Input [2]: [n_nationkey#1296L, n_name#1297]
Condition : isnotnull(n_nationkey#1296L)

(42) BroadcastExchange
Input [2]: [n_nationkey#1296L, n_name#1297]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, false]),false), [plan_id=15357]

(43) BroadcastHashJoin
Left keys [1]: [s_nationkey#211L]
Right keys [1]: [n_nationkey#1296L]
Join type: Inner
Join condition: None

(44) Project
Output [2]: [s_suppkey#208L, n_name#1297]
Input [4]: [s_suppkey#208L, s_nationkey#211L, n_nationkey#1296L, n_name#1297]

(45) Exchange
Input [2]: [s_suppkey#208L, n_name#1297]
Arguments: hashpartitioning(s_suppkey#208L, 200), ENSURE_REQUIREMENTS, [plan_id=15363]
...
(48) Project
Output [3]: [year(o_orderdate#20) AS o_year#1287, (l_extendedprice#30 * (1 - l_discount#31)) AS volume#1288, n_name#1297 AS nation#1289]
Input [6]: [o_orderdate#20, l_suppkey#27L, l_extendedprice#30, l_discount#31, s_suppkey#208L, n_name#1297]

(49) HashAggregate
Input [3]: [o_year#1287, volume#1288, nation#1289]
Keys [1]: [o_year#1287]
Functions [2]: [partial_sum(CASE WHEN (nation#1289 = CHINA) THEN volume#1288 ELSE 0 END), partial_sum(volume#1288)]
Aggregate Attributes [4]: [sum#1303, isEmpty#1304, sum#1305, isEmpty#1306]
Results [5]: [o_year#1287, sum#1307, isEmpty#1308, sum#1309, isEmpty#1310]

(50) Exchange
Input [5]: [o_year#1287, sum#1307, isEmpty#1308, sum#1309, isEmpty#1310]
Arguments: hashpartitioning(o_year#1287, 200), ENSURE_REQUIREMENTS, [plan_id=15370]

(51) HashAggregate
Input [5]: [o_year#1287, sum#1307, isEmpty#1308, sum#1309, isEmpty#1310]
Keys [1]: [o_year#1287]
Functions [2]: [sum(CASE WHEN (nation#1289 = CHINA) THEN volume#1288 ELSE 0 END), sum(volume#1288)]
Aggregate Attributes [2]: [sum(CASE WHEN (nation#1289 = CHINA) THEN volume#1288 ELSE 0 END)#1302, sum(volume#1288)#1301]
Results [2]: [o_year#1287, (sum(CASE WHEN (nation#1289 = CHINA) THEN volume#1288 ELSE 0 END)#1302 / sum(volume#1288)#1301) AS mkt_share#1290]

(52) Exchange
Input [2]: [o_year#1287, mkt_share#1290]
Arguments: rangepartitioning(o_year#1287 ASC NULLS FIRST, 200), ENSURE_REQUIREMENTS, [plan_id=15373]

(53) Sort
Input [2]: [o_year#1287, mkt_share#1290]
Arguments: [o_year#1287 ASC NULLS FIRST], true, 0

(54) AdaptiveSparkPlan
Output [2]: [o_year#1287, mkt_share#1290]
Arguments: isFinalPlan=false
Execution Time: 24.908
