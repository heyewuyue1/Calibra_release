...
PushedFilters: [IsNotNull(p_size), IsNotNull(p_type), EqualTo(p_size,14), StringEndsWith(p_type,NICKEL), IsNotNull(p_partkey)]
...
Condition : ((((isnotnull(p_size#204) AND isnotnull(p_type#203)) AND (p_size#204 = 14)) AND EndsWith(p_type#203, NICKEL)) AND isnotnull(p_partkey#199L))
...
Arguments: hashpartitioning(p_partkey#199L, 200), ENSURE_REQUIREMENTS, [plan_id=3400]
...
Output [2]: [n_nationkey#340L, n_regionkey#342L]
Batched: true
...
(7) Filter
Input [2]: [n_nationkey#340L, n_regionkey#342L]
Condition : (isnotnull(n_nationkey#340L) AND isnotnull(n_regionkey#342L))

(8) Scan parquet spark_catalog.tpch_sf100.region
Output [2]: [r_regionkey#344L, r_name#345]
Batched: true
...
(9) Filter
Input [2]: [r_regionkey#344L, r_name#345]
Condition : ((isnotnull(r_name#345) AND (r_name#345 = ASIA)) AND isnotnull(r_regionkey#344L))

(10) Project
Output [1]: [r_regionkey#344L]
Input [2]: [r_regionkey#344L, r_name#345]

(11) BroadcastExchange
Input [1]: [r_regionkey#344L]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=3378]

(12) BroadcastHashJoin
Left keys [1]: [n_regionkey#342L]
Right keys [1]: [r_regionkey#344L]
Join type: Inner
Join condition: None

(13) Project
Output [1]: [n_nationkey#340L]
Input [3]: [n_nationkey#340L, n_regionkey#342L, r_regionkey#344L]

(14) BroadcastExchange
Input [1]: [n_nationkey#340L]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=3382]

(15) Scan parquet spark_catalog.tpch_sf100.supplier
Output [2]: [s_suppkey#333L, s_nationkey#336L]
Batched: true
...
(16) Filter
Input [2]: [s_suppkey#333L, s_nationkey#336L]
Condition : (isnotnull(s_suppkey#333L) AND isnotnull(s_nationkey#336L))

(17) BroadcastHashJoin
Left keys [1]: [n_nationkey#340L]
Right keys [1]: [s_nationkey#336L]
Join type: Inner
Join condition: None

(18) Project
Output [1]: [s_suppkey#333L]
Input [3]: [n_nationkey#340L, s_suppkey#333L, s_nationkey#336L]

(19) Exchange
Input [1]: [s_suppkey#333L]
Arguments: hashpartitioning(s_suppkey#333L, 200), ENSURE_REQUIREMENTS, [plan_id=3387]

(20) Sort
Input [1]: [s_suppkey#333L]
Arguments: [s_suppkey#333L ASC NULLS FIRST], false, 0

(21) Scan parquet spark_catalog.tpch_sf100.partsupp
Output [3]: [ps_partkey#328L, ps_suppkey#329L, ps_supplycost#331]
Batched: true
...
(22) Filter
Input [3]: [ps_partkey#328L, ps_suppkey#329L, ps_supplycost#331]
Condition : (isnotnull(ps_suppkey#329L) AND isnotnull(ps_partkey#328L))

(23) Exchange
Input [3]: [ps_partkey#328L, ps_suppkey#329L, ps_supplycost#331]
Arguments: hashpartitioning(ps_suppkey#329L, 200), ENSURE_REQUIREMENTS, [plan_id=3388]

(24) Sort
Input [3]: [ps_partkey#328L, ps_suppkey#329L, ps_supplycost#331]
Arguments: [ps_suppkey#329L ASC NULLS FIRST], false, 0

(25) SortMergeJoin
Left keys [1]: [s_suppkey#333L]
Right keys [1]: [ps_suppkey#329L]
Join type: Inner
Join condition: None

(26) Project
Output [2]: [ps_partkey#328L, ps_supplycost#331]
Input [4]: [s_suppkey#333L, ps_partkey#328L, ps_suppkey#329L, ps_supplycost#331]

(27) HashAggregate
Input [2]: [ps_partkey#328L, ps_supplycost#331]
Keys [1]: [ps_partkey#328L]
Functions [1]: [partial_min(ps_supplycost#331)]
Aggregate Attributes [1]: [min#347]
Results [2]: [ps_partkey#328L, min#348]

(28) Exchange
Input [2]: [ps_partkey#328L, min#348]
Arguments: hashpartitioning(ps_partkey#328L, 200), ENSURE_REQUIREMENTS, [plan_id=3395]

(29) HashAggregate
Input [2]: [ps_partkey#328L, min#348]
Keys [1]: [ps_partkey#328L]
Functions [1]: [min(ps_supplycost#331)]
Aggregate Attributes [1]: [min(ps_supplycost#331)#326]
Results [2]: [min(ps_supplycost#331)#326 AS min(ps_supplycost)#327, ps_partkey#328L]

(30) Filter
Input [2]: [min(ps_supplycost)#327, ps_partkey#328L]
Condition : isnotnull(min(ps_supplycost)#327)

(31) Sort
Input [2]: [min(ps_supplycost)#327, ps_partkey#328L]
Arguments: [ps_partkey#328L ASC NULLS FIRST], false, 0

(32) SortMergeJoin
Left keys [1]: [p_partkey#199L]
Right keys [1]: [ps_partkey#328L]
Join type: Inner
Join condition: None

(33) Project
Output [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#327]
Input [4]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#327, ps_partkey#328L]

(34) Exchange
Input [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#327]
Arguments: hashpartitioning(p_partkey#199L, min(ps_supplycost)#327, 200), ENSURE_REQUIREMENTS, [plan_id=3408]

(35) Sort
Input [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#327]
Arguments: [p_partkey#199L ASC NULLS FIRST, min(ps_supplycost)#327 ASC NULLS FIRST], false, 0
...
(38) Exchange
Input [3]: [ps_partkey#215L, ps_suppkey#216L, ps_supplycost#218]
Arguments: hashpartitioning(ps_partkey#215L, ps_supplycost#218, 200), ENSURE_REQUIREMENTS, [plan_id=3407]
...
(40) SortMergeJoin
Left keys [2]: [p_partkey#199L, min(ps_supplycost)#327]
...
Input [6]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#327, ps_partkey#215L, ps_suppkey#216L, ps_supplycost#218]

(42) Exchange
Input [3]: [p_partkey#199L, p_mfgr#201, ps_suppkey#216L]
Arguments: hashpartitioning(ps_suppkey#216L, 200), ENSURE_REQUIREMENTS, [plan_id=3422]
...
Condition : ((isnotnull(r_name#221) AND (r_name#221 = ASIA)) AND isnotnull(r_regionkey#220L))
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=3413]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=3417]
...
Arguments: hashpartitioning(s_suppkey#208L, 200), ENSURE_REQUIREMENTS, [plan_id=3423]
