...
PushedFilters: [IsNotNull(p_size), IsNotNull(p_type), EqualTo(p_size,38), StringEndsWith(p_type,STEEL), IsNotNull(p_partkey)]
...
Condition : ((((isnotnull(p_size#204) AND isnotnull(p_type#203)) AND (p_size#204 = 38)) AND EndsWith(p_type#203, STEEL)) AND isnotnull(p_partkey#199L))
...
Arguments: hashpartitioning(p_partkey#199L, 200), ENSURE_REQUIREMENTS, [plan_id=2242]
...
Output [2]: [n_nationkey#241L, n_regionkey#243L]
Batched: true
...
(7) Filter
Input [2]: [n_nationkey#241L, n_regionkey#243L]
Condition : (isnotnull(n_nationkey#241L) AND isnotnull(n_regionkey#243L))

(8) Scan parquet spark_catalog.tpch_sf100.region
Output [2]: [r_regionkey#245L, r_name#246]
Batched: true
...
(9) Filter
Input [2]: [r_regionkey#245L, r_name#246]
Condition : ((isnotnull(r_name#246) AND (r_name#246 = ASIA)) AND isnotnull(r_regionkey#245L))

(10) Project
Output [1]: [r_regionkey#245L]
Input [2]: [r_regionkey#245L, r_name#246]

(11) BroadcastExchange
Input [1]: [r_regionkey#245L]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=2220]

(12) BroadcastHashJoin
Left keys [1]: [n_regionkey#243L]
Right keys [1]: [r_regionkey#245L]
Join type: Inner
Join condition: None

(13) Project
Output [1]: [n_nationkey#241L]
Input [3]: [n_nationkey#241L, n_regionkey#243L, r_regionkey#245L]

(14) BroadcastExchange
Input [1]: [n_nationkey#241L]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=2224]

(15) Scan parquet spark_catalog.tpch_sf100.supplier
Output [2]: [s_suppkey#234L, s_nationkey#237L]
Batched: true
...
(16) Filter
Input [2]: [s_suppkey#234L, s_nationkey#237L]
Condition : (isnotnull(s_suppkey#234L) AND isnotnull(s_nationkey#237L))

(17) BroadcastHashJoin
Left keys [1]: [n_nationkey#241L]
Right keys [1]: [s_nationkey#237L]
Join type: Inner
Join condition: None

(18) Project
Output [1]: [s_suppkey#234L]
Input [3]: [n_nationkey#241L, s_suppkey#234L, s_nationkey#237L]

(19) Exchange
Input [1]: [s_suppkey#234L]
Arguments: hashpartitioning(s_suppkey#234L, 200), ENSURE_REQUIREMENTS, [plan_id=2229]

(20) Sort
Input [1]: [s_suppkey#234L]
Arguments: [s_suppkey#234L ASC NULLS FIRST], false, 0

(21) Scan parquet spark_catalog.tpch_sf100.partsupp
Output [3]: [ps_partkey#229L, ps_suppkey#230L, ps_supplycost#232]
Batched: true
...
(22) Filter
Input [3]: [ps_partkey#229L, ps_suppkey#230L, ps_supplycost#232]
Condition : (isnotnull(ps_suppkey#230L) AND isnotnull(ps_partkey#229L))

(23) Exchange
Input [3]: [ps_partkey#229L, ps_suppkey#230L, ps_supplycost#232]
Arguments: hashpartitioning(ps_suppkey#230L, 200), ENSURE_REQUIREMENTS, [plan_id=2230]

(24) Sort
Input [3]: [ps_partkey#229L, ps_suppkey#230L, ps_supplycost#232]
Arguments: [ps_suppkey#230L ASC NULLS FIRST], false, 0

(25) SortMergeJoin
Left keys [1]: [s_suppkey#234L]
Right keys [1]: [ps_suppkey#230L]
Join type: Inner
Join condition: None

(26) Project
Output [2]: [ps_partkey#229L, ps_supplycost#232]
Input [4]: [s_suppkey#234L, ps_partkey#229L, ps_suppkey#230L, ps_supplycost#232]

(27) HashAggregate
Input [2]: [ps_partkey#229L, ps_supplycost#232]
Keys [1]: [ps_partkey#229L]
Functions [1]: [partial_min(ps_supplycost#232)]
Aggregate Attributes [1]: [min#248]
Results [2]: [ps_partkey#229L, min#249]

(28) Exchange
Input [2]: [ps_partkey#229L, min#249]
Arguments: hashpartitioning(ps_partkey#229L, 200), ENSURE_REQUIREMENTS, [plan_id=2237]

(29) HashAggregate
Input [2]: [ps_partkey#229L, min#249]
Keys [1]: [ps_partkey#229L]
Functions [1]: [min(ps_supplycost#232)]
Aggregate Attributes [1]: [min(ps_supplycost#232)#226]
Results [2]: [min(ps_supplycost#232)#226 AS min(ps_supplycost)#227, ps_partkey#229L]

(30) Filter
Input [2]: [min(ps_supplycost)#227, ps_partkey#229L]
Condition : isnotnull(min(ps_supplycost)#227)

(31) Sort
Input [2]: [min(ps_supplycost)#227, ps_partkey#229L]
Arguments: [ps_partkey#229L ASC NULLS FIRST], false, 0

(32) SortMergeJoin
Left keys [1]: [p_partkey#199L]
Right keys [1]: [ps_partkey#229L]
Join type: Inner
Join condition: None

(33) Project
Output [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#227]
Input [4]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#227, ps_partkey#229L]

(34) Exchange
Input [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#227]
Arguments: hashpartitioning(p_partkey#199L, min(ps_supplycost)#227, 200), ENSURE_REQUIREMENTS, [plan_id=2250]

(35) Sort
Input [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#227]
Arguments: [p_partkey#199L ASC NULLS FIRST, min(ps_supplycost)#227 ASC NULLS FIRST], false, 0
...
(38) Exchange
Input [3]: [ps_partkey#215L, ps_suppkey#216L, ps_supplycost#218]
Arguments: hashpartitioning(ps_partkey#215L, ps_supplycost#218, 200), ENSURE_REQUIREMENTS, [plan_id=2249]
...
(40) SortMergeJoin
Left keys [2]: [p_partkey#199L, min(ps_supplycost)#227]
...
Input [6]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#227, ps_partkey#215L, ps_suppkey#216L, ps_supplycost#218]

(42) Exchange
Input [3]: [p_partkey#199L, p_mfgr#201, ps_suppkey#216L]
Arguments: hashpartitioning(ps_suppkey#216L, 200), ENSURE_REQUIREMENTS, [plan_id=2264]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=2255]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=2259]
...
Arguments: hashpartitioning(s_suppkey#208L, 200), ENSURE_REQUIREMENTS, [plan_id=2265]
