...
PushedFilters: [IsNotNull(r_name), EqualTo(r_name,MIDDLE EAST), IsNotNull(r_regionkey)]
ReadSchema: struct<r_regionkey:bigint,r_name:string>

(4) Filter
Input [2]: [r_regionkey#220L, r_name#221]
Condition : ((isnotnull(r_name#221) AND (r_name#221 = MIDDLE EAST)) AND isnotnull(r_regionkey#220L))
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=15681]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=15685]
...
Arguments: hashpartitioning(c_custkey#8L, 200), ENSURE_REQUIREMENTS, [plan_id=15690]
...
(18) Exchange
Input [3]: [o_orderkey#16L, o_custkey#17L, o_orderdate#20]
Arguments: hashpartitioning(o_custkey#17L, 200), ENSURE_REQUIREMENTS, [plan_id=15691]
...
Arguments: hashpartitioning(o_orderkey#16L, 200), ENSURE_REQUIREMENTS, [plan_id=15701]
...
PushedFilters: [IsNotNull(p_type), EqualTo(p_type,MEDIUM BRUSHED NICKEL), IsNotNull(p_partkey)]
ReadSchema: struct<p_partkey:bigint,p_type:string>

(25) Filter
Input [2]: [p_partkey#199L, p_type#203]
Condition : ((isnotnull(p_type#203) AND (p_type#203 = MEDIUM BRUSHED NICKEL)) AND isnotnull(p_partkey#199L))
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=15696]
...
Arguments: hashpartitioning(l_orderkey#25L, 200), ENSURE_REQUIREMENTS, [plan_id=15702]
...
Arguments: hashpartitioning(l_suppkey#27L, 200), ENSURE_REQUIREMENTS, [plan_id=15712]
...
Output [2]: [n_nationkey#1324L, n_name#1325]
Batched: true
Location: InMemoryFileIndex [file:/path/to/datasets/tpch_sf100_parquet/nation]
...
(41) Filter
Input [2]: [n_nationkey#1324L, n_name#1325]
Condition : isnotnull(n_nationkey#1324L)

(42) BroadcastExchange
Input [2]: [n_nationkey#1324L, n_name#1325]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, false]),false), [plan_id=15707]

(43) BroadcastHashJoin
Left keys [1]: [s_nationkey#211L]
Right keys [1]: [n_nationkey#1324L]
Join type: Inner
Join condition: None

(44) Project
Output [2]: [s_suppkey#208L, n_name#1325]
Input [4]: [s_suppkey#208L, s_nationkey#211L, n_nationkey#1324L, n_name#1325]

(45) Exchange
Input [2]: [s_suppkey#208L, n_name#1325]
Arguments: hashpartitioning(s_suppkey#208L, 200), ENSURE_REQUIREMENTS, [plan_id=15713]
...
(48) Project
Output [3]: [year(o_orderdate#20) AS o_year#1315, (l_extendedprice#30 * (1 - l_discount#31)) AS volume#1316, n_name#1325 AS nation#1317]
Input [6]: [o_orderdate#20, l_suppkey#27L, l_extendedprice#30, l_discount#31, s_suppkey#208L, n_name#1325]

(49) HashAggregate
Input [3]: [o_year#1315, volume#1316, nation#1317]
Keys [1]: [o_year#1315]
Functions [2]: [partial_sum(CASE WHEN (nation#1317 = IRAN) THEN volume#1316 ELSE 0 END), partial_sum(volume#1316)]
Aggregate Attributes [4]: [sum#1331, isEmpty#1332, sum#1333, isEmpty#1334]
Results [5]: [o_year#1315, sum#1335, isEmpty#1336, sum#1337, isEmpty#1338]

(50) Exchange
Input [5]: [o_year#1315, sum#1335, isEmpty#1336, sum#1337, isEmpty#1338]
Arguments: hashpartitioning(o_year#1315, 200), ENSURE_REQUIREMENTS, [plan_id=15720]

(51) HashAggregate
Input [5]: [o_year#1315, sum#1335, isEmpty#1336, sum#1337, isEmpty#1338]
Keys [1]: [o_year#1315]
Functions [2]: [sum(CASE WHEN (nation#1317 = IRAN) THEN volume#1316 ELSE 0 END), sum(volume#1316)]
Aggregate Attributes [2]: [sum(CASE WHEN (nation#1317 = IRAN) THEN volume#1316 ELSE 0 END)#1330, sum(volume#1316)#1329]
Results [2]: [o_year#1315, (sum(CASE WHEN (nation#1317 = IRAN) THEN volume#1316 ELSE 0 END)#1330 / sum(volume#1316)#1329) AS mkt_share#1318]

(52) Exchange
Input [2]: [o_year#1315, mkt_share#1318]
Arguments: rangepartitioning(o_year#1315 ASC NULLS FIRST, 200), ENSURE_REQUIREMENTS, [plan_id=15723]

(53) Sort
Input [2]: [o_year#1315, mkt_share#1318]
Arguments: [o_year#1315 ASC NULLS FIRST], true, 0

(54) AdaptiveSparkPlan
Output [2]: [o_year#1315, mkt_share#1318]
Arguments: isFinalPlan=false
Execution Time: 25.692
