...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=12881]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=12885]
...
Arguments: hashpartitioning(c_custkey#8L, 200), ENSURE_REQUIREMENTS, [plan_id=12890]
...
(18) Exchange
Input [3]: [o_orderkey#16L, o_custkey#17L, o_orderdate#20]
Arguments: hashpartitioning(o_custkey#17L, 200), ENSURE_REQUIREMENTS, [plan_id=12891]
...
Arguments: hashpartitioning(o_orderkey#16L, 200), ENSURE_REQUIREMENTS, [plan_id=12901]
...
PushedFilters: [IsNotNull(p_type), EqualTo(p_type,PROMO POLISHED TIN), IsNotNull(p_partkey)]
ReadSchema: struct<p_partkey:bigint,p_type:string>

(25) Filter
Input [2]: [p_partkey#199L, p_type#203]
Condition : ((isnotnull(p_type#203) AND (p_type#203 = PROMO POLISHED TIN)) AND isnotnull(p_partkey#199L))
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=12896]
...
Arguments: hashpartitioning(l_orderkey#25L, 200), ENSURE_REQUIREMENTS, [plan_id=12902]
...
Arguments: hashpartitioning(l_suppkey#27L, 200), ENSURE_REQUIREMENTS, [plan_id=12912]
...
Output [2]: [n_nationkey#1100L, n_name#1101]
Batched: true
Location: InMemoryFileIndex [file:/path/to/datasets/tpch_sf100_parquet/nation]
...
(41) Filter
Input [2]: [n_nationkey#1100L, n_name#1101]
Condition : isnotnull(n_nationkey#1100L)

(42) BroadcastExchange
Input [2]: [n_nationkey#1100L, n_name#1101]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, false]),false), [plan_id=12907]

(43) BroadcastHashJoin
Left keys [1]: [s_nationkey#211L]
Right keys [1]: [n_nationkey#1100L]
Join type: Inner
Join condition: None

(44) Project
Output [2]: [s_suppkey#208L, n_name#1101]
Input [4]: [s_suppkey#208L, s_nationkey#211L, n_nationkey#1100L, n_name#1101]

(45) Exchange
Input [2]: [s_suppkey#208L, n_name#1101]
Arguments: hashpartitioning(s_suppkey#208L, 200), ENSURE_REQUIREMENTS, [plan_id=12913]
...
(48) Project
Output [3]: [year(o_orderdate#20) AS o_year#1091, (l_extendedprice#30 * (1 - l_discount#31)) AS volume#1092, n_name#1101 AS nation#1093]
Input [6]: [o_orderdate#20, l_suppkey#27L, l_extendedprice#30, l_discount#31, s_suppkey#208L, n_name#1101]

(49) HashAggregate
Input [3]: [o_year#1091, volume#1092, nation#1093]
Keys [1]: [o_year#1091]
Functions [2]: [partial_sum(CASE WHEN (nation#1093 = MOZAMBIQUE) THEN volume#1092 ELSE 0 END), partial_sum(volume#1092)]
Aggregate Attributes [4]: [sum#1107, isEmpty#1108, sum#1109, isEmpty#1110]
Results [5]: [o_year#1091, sum#1111, isEmpty#1112, sum#1113, isEmpty#1114]

(50) Exchange
Input [5]: [o_year#1091, sum#1111, isEmpty#1112, sum#1113, isEmpty#1114]
Arguments: hashpartitioning(o_year#1091, 200), ENSURE_REQUIREMENTS, [plan_id=12920]

(51) HashAggregate
Input [5]: [o_year#1091, sum#1111, isEmpty#1112, sum#1113, isEmpty#1114]
Keys [1]: [o_year#1091]
Functions [2]: [sum(CASE WHEN (nation#1093 = MOZAMBIQUE) THEN volume#1092 ELSE 0 END), sum(volume#1092)]
Aggregate Attributes [2]: [sum(CASE WHEN (nation#1093 = MOZAMBIQUE) THEN volume#1092 ELSE 0 END)#1106, sum(volume#1092)#1105]
Results [2]: [o_year#1091, (sum(CASE WHEN (nation#1093 = MOZAMBIQUE) THEN volume#1092 ELSE 0 END)#1106 / sum(volume#1092)#1105) AS mkt_share#1094]

(52) Exchange
Input [2]: [o_year#1091, mkt_share#1094]
Arguments: rangepartitioning(o_year#1091 ASC NULLS FIRST, 200), ENSURE_REQUIREMENTS, [plan_id=12923]

(53) Sort
Input [2]: [o_year#1091, mkt_share#1094]
Arguments: [o_year#1091 ASC NULLS FIRST], true, 0

(54) AdaptiveSparkPlan
Output [2]: [o_year#1091, mkt_share#1094]
Arguments: isFinalPlan=false
Execution Time: 25.528
