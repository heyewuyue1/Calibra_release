...
PushedFilters: [IsNotNull(p_size), IsNotNull(p_type), EqualTo(p_size,15), StringEndsWith(p_type,NICKEL), IsNotNull(p_partkey)]
...
Condition : ((((isnotnull(p_size#204) AND isnotnull(p_type#203)) AND (p_size#204 = 15)) AND EndsWith(p_type#203, NICKEL)) AND isnotnull(p_partkey#199L))
...
Arguments: hashpartitioning(p_partkey#199L, 200), ENSURE_REQUIREMENTS, [plan_id=4944]
...
Output [2]: [n_nationkey#472L, n_regionkey#474L]
Batched: true
...
(7) Filter
Input [2]: [n_nationkey#472L, n_regionkey#474L]
Condition : (isnotnull(n_nationkey#472L) AND isnotnull(n_regionkey#474L))

(8) Scan parquet spark_catalog.tpch_sf100.region
Output [2]: [r_regionkey#476L, r_name#477]
Batched: true
...
(9) Filter
Input [2]: [r_regionkey#476L, r_name#477]
Condition : ((isnotnull(r_name#477) AND (r_name#477 = EUROPE)) AND isnotnull(r_regionkey#476L))

(10) Project
Output [1]: [r_regionkey#476L]
Input [2]: [r_regionkey#476L, r_name#477]

(11) BroadcastExchange
Input [1]: [r_regionkey#476L]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=4922]

(12) BroadcastHashJoin
Left keys [1]: [n_regionkey#474L]
Right keys [1]: [r_regionkey#476L]
Join type: Inner
Join condition: None

(13) Project
Output [1]: [n_nationkey#472L]
Input [3]: [n_nationkey#472L, n_regionkey#474L, r_regionkey#476L]

(14) BroadcastExchange
Input [1]: [n_nationkey#472L]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=4926]

(15) Scan parquet spark_catalog.tpch_sf100.supplier
Output [2]: [s_suppkey#465L, s_nationkey#468L]
Batched: true
...
(16) Filter
Input [2]: [s_suppkey#465L, s_nationkey#468L]
Condition : (isnotnull(s_suppkey#465L) AND isnotnull(s_nationkey#468L))

(17) BroadcastHashJoin
Left keys [1]: [n_nationkey#472L]
Right keys [1]: [s_nationkey#468L]
Join type: Inner
Join condition: None

(18) Project
Output [1]: [s_suppkey#465L]
Input [3]: [n_nationkey#472L, s_suppkey#465L, s_nationkey#468L]

(19) Exchange
Input [1]: [s_suppkey#465L]
Arguments: hashpartitioning(s_suppkey#465L, 200), ENSURE_REQUIREMENTS, [plan_id=4931]

(20) Sort
Input [1]: [s_suppkey#465L]
Arguments: [s_suppkey#465L ASC NULLS FIRST], false, 0

(21) Scan parquet spark_catalog.tpch_sf100.partsupp
Output [3]: [ps_partkey#460L, ps_suppkey#461L, ps_supplycost#463]
Batched: true
...
(22) Filter
Input [3]: [ps_partkey#460L, ps_suppkey#461L, ps_supplycost#463]
Condition : (isnotnull(ps_suppkey#461L) AND isnotnull(ps_partkey#460L))

(23) Exchange
Input [3]: [ps_partkey#460L, ps_suppkey#461L, ps_supplycost#463]
Arguments: hashpartitioning(ps_suppkey#461L, 200), ENSURE_REQUIREMENTS, [plan_id=4932]

(24) Sort
Input [3]: [ps_partkey#460L, ps_suppkey#461L, ps_supplycost#463]
Arguments: [ps_suppkey#461L ASC NULLS FIRST], false, 0

(25) SortMergeJoin
Left keys [1]: [s_suppkey#465L]
Right keys [1]: [ps_suppkey#461L]
Join type: Inner
Join condition: None

(26) Project
Output [2]: [ps_partkey#460L, ps_supplycost#463]
Input [4]: [s_suppkey#465L, ps_partkey#460L, ps_suppkey#461L, ps_supplycost#463]

(27) HashAggregate
Input [2]: [ps_partkey#460L, ps_supplycost#463]
Keys [1]: [ps_partkey#460L]
Functions [1]: [partial_min(ps_supplycost#463)]
Aggregate Attributes [1]: [min#479]
Results [2]: [ps_partkey#460L, min#480]

(28) Exchange
Input [2]: [ps_partkey#460L, min#480]
Arguments: hashpartitioning(ps_partkey#460L, 200), ENSURE_REQUIREMENTS, [plan_id=4939]

(29) HashAggregate
Input [2]: [ps_partkey#460L, min#480]
Keys [1]: [ps_partkey#460L]
Functions [1]: [min(ps_supplycost#463)]
Aggregate Attributes [1]: [min(ps_supplycost#463)#458]
Results [2]: [min(ps_supplycost#463)#458 AS min(ps_supplycost)#459, ps_partkey#460L]

(30) Filter
Input [2]: [min(ps_supplycost)#459, ps_partkey#460L]
Condition : isnotnull(min(ps_supplycost)#459)

(31) Sort
Input [2]: [min(ps_supplycost)#459, ps_partkey#460L]
Arguments: [ps_partkey#460L ASC NULLS FIRST], false, 0

(32) SortMergeJoin
Left keys [1]: [p_partkey#199L]
Right keys [1]: [ps_partkey#460L]
Join type: Inner
Join condition: None

(33) Project
Output [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#459]
Input [4]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#459, ps_partkey#460L]

(34) Exchange
Input [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#459]
Arguments: hashpartitioning(p_partkey#199L, min(ps_supplycost)#459, 200), ENSURE_REQUIREMENTS, [plan_id=4952]

(35) Sort
Input [3]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#459]
Arguments: [p_partkey#199L ASC NULLS FIRST, min(ps_supplycost)#459 ASC NULLS FIRST], false, 0
...
(38) Exchange
Input [3]: [ps_partkey#215L, ps_suppkey#216L, ps_supplycost#218]
Arguments: hashpartitioning(ps_partkey#215L, ps_supplycost#218, 200), ENSURE_REQUIREMENTS, [plan_id=4951]
...
(40) SortMergeJoin
Left keys [2]: [p_partkey#199L, min(ps_supplycost)#459]
...
Input [6]: [p_partkey#199L, p_mfgr#201, min(ps_supplycost)#459, ps_partkey#215L, ps_suppkey#216L, ps_supplycost#218]

(42) Exchange
Input [3]: [p_partkey#199L, p_mfgr#201, ps_suppkey#216L]
Arguments: hashpartitioning(ps_suppkey#216L, 200), ENSURE_REQUIREMENTS, [plan_id=4966]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=4957]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=4961]
...
Arguments: hashpartitioning(s_suppkey#208L, 200), ENSURE_REQUIREMENTS, [plan_id=4967]
