...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=13931]
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=13935]
...
Arguments: hashpartitioning(c_custkey#8L, 200), ENSURE_REQUIREMENTS, [plan_id=13940]
...
(18) Exchange
Input [3]: [o_orderkey#16L, o_custkey#17L, o_orderdate#20]
Arguments: hashpartitioning(o_custkey#17L, 200), ENSURE_REQUIREMENTS, [plan_id=13941]
...
Arguments: hashpartitioning(o_orderkey#16L, 200), ENSURE_REQUIREMENTS, [plan_id=13951]
...
PushedFilters: [IsNotNull(p_type), EqualTo(p_type,ECONOMY BRUSHED TIN), IsNotNull(p_partkey)]
ReadSchema: struct<p_partkey:bigint,p_type:string>

(25) Filter
Input [2]: [p_partkey#199L, p_type#203]
Condition : ((isnotnull(p_type#203) AND (p_type#203 = ECONOMY BRUSHED TIN)) AND isnotnull(p_partkey#199L))
...
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, true]),false), [plan_id=13946]
...
Arguments: hashpartitioning(l_orderkey#25L, 200), ENSURE_REQUIREMENTS, [plan_id=13952]
...
Arguments: hashpartitioning(l_suppkey#27L, 200), ENSURE_REQUIREMENTS, [plan_id=13962]
...
Output [2]: [n_nationkey#1184L, n_name#1185]
Batched: true
Location: InMemoryFileIndex [file:/path/to/datasets/tpch_sf100_parquet/nation]
...
(41) Filter
Input [2]: [n_nationkey#1184L, n_name#1185]
Condition : isnotnull(n_nationkey#1184L)

(42) BroadcastExchange
Input [2]: [n_nationkey#1184L, n_name#1185]
Arguments: HashedRelationBroadcastMode(List(input[0, bigint, false]),false), [plan_id=13957]

(43) BroadcastHashJoin
Left keys [1]: [s_nationkey#211L]
Right keys [1]: [n_nationkey#1184L]
Join type: Inner
Join condition: None

(44) Project
Output [2]: [s_suppkey#208L, n_name#1185]
Input [4]: [s_suppkey#208L, s_nationkey#211L, n_nationkey#1184L, n_name#1185]

(45) Exchange
Input [2]: [s_suppkey#208L, n_name#1185]
Arguments: hashpartitioning(s_suppkey#208L, 200), ENSURE_REQUIREMENTS, [plan_id=13963]
...
(48) Project
Output [3]: [year(o_orderdate#20) AS o_year#1175, (l_extendedprice#30 * (1 - l_discount#31)) AS volume#1176, n_name#1185 AS nation#1177]
Input [6]: [o_orderdate#20, l_suppkey#27L, l_extendedprice#30, l_discount#31, s_suppkey#208L, n_name#1185]

(49) HashAggregate
Input [3]: [o_year#1175, volume#1176, nation#1177]
Keys [1]: [o_year#1175]
Functions [2]: [partial_sum(CASE WHEN (nation#1177 = ALGERIA) THEN volume#1176 ELSE 0 END), partial_sum(volume#1176)]
Aggregate Attributes [4]: [sum#1191, isEmpty#1192, sum#1193, isEmpty#1194]
Results [5]: [o_year#1175, sum#1195, isEmpty#1196, sum#1197, isEmpty#1198]

(50) Exchange
Input [5]: [o_year#1175, sum#1195, isEmpty#1196, sum#1197, isEmpty#1198]
Arguments: hashpartitioning(o_year#1175, 200), ENSURE_REQUIREMENTS, [plan_id=13970]

(51) HashAggregate
Input [5]: [o_year#1175, sum#1195, isEmpty#1196, sum#1197, isEmpty#1198]
Keys [1]: [o_year#1175]
Functions [2]: [sum(CASE WHEN (nation#1177 = ALGERIA) THEN volume#1176 ELSE 0 END), sum(volume#1176)]
Aggregate Attributes [2]: [sum(CASE WHEN (nation#1177 = ALGERIA) THEN volume#1176 ELSE 0 END)#1190, sum(volume#1176)#1189]
Results [2]: [o_year#1175, (sum(CASE WHEN (nation#1177 = ALGERIA) THEN volume#1176 ELSE 0 END)#1190 / sum(volume#1176)#1189) AS mkt_share#1178]

(52) Exchange
Input [2]: [o_year#1175, mkt_share#1178]
Arguments: rangepartitioning(o_year#1175 ASC NULLS FIRST, 200), ENSURE_REQUIREMENTS, [plan_id=13973]

(53) Sort
Input [2]: [o_year#1175, mkt_share#1178]
Arguments: [o_year#1175 ASC NULLS FIRST], true, 0

(54) AdaptiveSparkPlan
Output [2]: [o_year#1175, mkt_share#1178]
Arguments: isFinalPlan=false
Execution Time: 25.671
